    ...
    modal.innerHTML = `
        ...
        <div class="flex items-center justify-between p-3 bg-gray-900 text-white">
            ...
            <div class="flex gap-2">
                ${allowDownload ? `<a href="${pdfUrl}" download class="bg-green-600 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-green-700 transition">
        ...
        <!-- Zoom Tip -->
        <div class="bg-gray-800 text-center py-2 text-xs text-gray-400">
...
window.openFullscreenPDF = (pdfUrl, allowDownload = true) => {
    ...
    modal.innerHTML = `
        <!-- Header -->
        <div class="flex items-center justify-between p-3 bg-gradient-to-r from-blue-900 to-blue-900 text-white shadow-lg">
            <div class="flex items-center gap-3">
                <i class="fas fa-book-reader text-2xl text-blue-300"></i>
                <div>
                    <span class="font-bold text-lg">وضع القراءة</span>
                    <p class="text-xs text-blue-200 opacity-80">${isDriveLink ? 'Google Drive Viewer' : 'يمكنك التكبير والتصغير بحرية'}</p>
                </div>
            </div>
            <div class="flex gap-2">
                ${allowDownload ? `<a href="${downloadUrl}" ${isDriveLink ? 'target="_blank"' : 'download'} class="bg-green-600 hover:bg-green-700 px-4 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2 shadow-lg">
                    <i class="fas fa-download"></i>
                    <span class="hidden sm:inline">تحميل</span>
                </a>
                <a href="${pdfUrl}" target="_blank" class="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2 shadow-lg">
        ...
        <div class="flex-1 relative bg-gray-900">
            ${isDriveLink ? `
                ...
                <div class="absolute top-0 right-0 w-20 h-14 bg-gray-900 z-10"></div>
                <iframe 
                    src="${embedUrl}" 
                    class="w-full h-full border-0"
...
const transformUrlForEmbed = (url, allowDownload = true) => {
    ...
    if (processedUrl.includes('drive.google.com') || processedUrl.includes('docs.google.com') || processedUrl.match(/\/d\/([a-zA-Z0-9_-]{25,})/)) {
        ...
        return `<div class="mt-6 border-2 border-blue-50 rounded-2xl shadow-inner bg-gray-50 dark:bg-gray-800 overflow-hidden relative">
            <!-- Header with buttons -->
            <div class="flex items-center justify-between p-3 bg-gray-100 dark:bg-gray-700 border-b dark:border-gray-600">
                <span class="text-sm font-bold text-gray-600 dark:text-gray-300"><i class="fab fa-google-drive text-blue-500 ml-2"></i>ملف Google Drive</span>
                <div class="flex gap-2 flex-wrap">
                    <button onclick="window.openFullscreenPDF('${processedUrl}', ${allowDownload})" class="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-blue-700 transition">
                        <i class="fas fa-book-reader ml-1"></i>وضع القراءة
                    </button>
                    ${allowDownload ? `<a href="${processedUrl}" target="_blank" class="bg-green-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-green-700 transition">
                        <i class="fas fa-download ml-1"></i>تحميل
                    </a>` : ''}
                </div>
            </div>
            ...
            <div class="h-[500px] md:h-[600px] relative">
                <div class="absolute top-0 right-0 w-16 h-14 bg-gray-100 dark:bg-gray-700 z-10 rounded-bl-xl"></div>
    ...
    if (url.includes('youtube.com') || url.includes('youtu.be')) {
        ...
        return `<div class="mt-6 aspect-video rounded-2xl overflow-hidden shadow-xl border-4 border-white dark:border-gray-700"><iframe class="w-full h-full" src="https://www.youtube.com/embed/${vId}" allowfullscreen></iframe></div>`;
    }
    if (lowerUrl.match(/\.(mp4|webm|ogg)$/i)) {
        return `<video controls class="w-full mt-6 rounded-2xl shadow-lg border-4 border-white dark:border-gray-700"><source src="${url}">متصفحك لا يدعم الفيديو.</video>`;
    }
    if (lowerUrl.includes('.pdf')) {
        ...
        return `
            <div class="mt-6 border rounded-2xl overflow-hidden shadow-lg bg-gray-50 dark:bg-gray-800">
                <div class="flex items-center justify-between p-3 bg-gray-100 dark:bg-gray-700 border-b dark:border-gray-600 flex-wrap gap-2">
                    <span class="text-sm font-bold text-gray-600 dark:text-gray-300"><i class="fas fa-file-pdf text-red-500 ml-2"></i>ملف PDF</span>
                    <div class="flex gap-2 flex-wrap">
                        <button onclick="window.openPDFFullscreen('${url}', ${allowDownload})" class="bg-indigo-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-indigo-700 transition">
                            <i class="fas fa-expand ml-1"></i>فتح ملء الشاشة (Zoom)
                        </button>
                        ${allowDownload ? `<a href="${url}" download class="bg-green-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-green-700 transition">
                            <i class="fas fa-download ml-1"></i>تحميل
                        </a>` : ''}
                    </div>
                </div>
                <div class="h-[400px] md:h-[600px] bg-gray-200 dark:bg-gray-900 flex items-center justify-center">
                    <button onclick="window.openPDFFullscreen('${url}', ${allowDownload})" class="bg-white dark:bg-gray-800 px-8 py-6 rounded-2xl shadow-lg hover:scale-105 transition text-center">
                        <i class="fas fa-file-pdf text-6xl text-red-500 mb-4"></i>
                        <p class="font-bold text-gray-700 dark:text-gray-200">اضغط لعرض الـ PDF</p>
                        <p class="text-xs text-gray-400 mt-2">🔍 يدعم الزوم بالصابعين</p>
                    </button>
                </div>
            </div>
        `;
    ...
    if (lowerUrl.match(/\.(jpeg|jpg|gif|png|webp)$/i)) {
        return `<img src="${url}" loading="lazy" class="max-w-full rounded-2xl shadow-lg mx-auto mt-6 border-4 border-white dark:border-gray-700 hover:scale-[1.01] transition transform cursor-pointer" onclick="window.open('${url}')">`;
    }
    return `<a href="${url}" target="_blank" class="flex items-center gap-3 mt-6 p-4 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-xl border border-blue-100 dark:border-blue-800 transition hover:bg-blue-100 hover:scale-[1.02] transform"><i class="fas fa-external-link-alt text-xl"></i> <span class="font-bold underline">فتح الملف المرفق</span></a>`;
};
...
export const setupNotificationListener = (userId) => {
    ...
    onSnapshot(q, (snap) => {
        ...
        if (snap.empty) {
            html = '<div class="p-8 text-center text-gray-400 text-sm"><i class="fas fa-bell-slash text-2xl mb-2 block opacity-20"></i>لا توجد تنبيهات جديدة</div>';
        } else {
            snap.forEach(d => {
                const n = d.data();
                if (!n.read) unread++;
                html += `
                    <div class="p-4 border-b dark:border-gray-700 hover:bg-blue-50 dark:hover:bg-blue-900/20 cursor-pointer transition ${!n.read ? 'bg-blue-50/50 dark:bg-blue-500/10 border-r-4 border-r-blue-500' : ''}" 
                         onclick="window.markNotifRead('${d.id}', '${n.link}')">
                        <p class="text-sm font-bold dark:text-white leading-tight">${n.message}</p>
                        <span class="text-[10px] text-gray-400 mt-1 block font-mono">${n.createdAt?.toDate().toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>`;
            });
        }
        list.innerHTML = html;
...
export const renderSubsectionsForUser = async (sectionId) => {
    ...
    contentDiv.innerHTML = '<div class="text-center p-20"><i class="fas fa-circle-notch fa-spin text-5xl text-blue-600"></i><p class="mt-4 font-bold text-gray-500">جاري تحضير الدرس...</p></div>';
    ...
    try {
        ...
        if (snapshot.empty) return contentDiv.innerHTML = '<div class="p-20 text-center bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-dashed border-gray-300 dark:border-gray-700 text-gray-500"><i class="fas fa-folder-open text-6xl mb-4 opacity-20"></i><p class="text-xl">لا يوجد محتوى مضاف حالياً.</p></div>';
        ...
        snapshot.forEach(async (docSnap) => {
            ...
            sectionCard.className = "bg-white dark:bg-gray-800 p-8 rounded-[2.5rem] shadow-xl mb-12 border border-gray-100 dark:border-gray-700 animate-fade-in relative overflow-hidden";

            let htmlContent = `
                <div class="flex justify-between items-start mb-8 relative z-10">
                    <h2 class="text-3xl font-black text-blue-800 dark:text-blue-400 flex items-center gap-3">
                        <span class="bg-blue-100 dark:bg-blue-900/50 w-12 h-12 rounded-xl flex items-center justify-center text-2xl shadow-sm">
                            ${data.isBookMode ? '📘' : '📺'}
                        </span>
                        ${data.title}
                    </h2>
                    <div class="flex items-center gap-2">
                        ${data.isBookMode && data.fileUrl ? `<button onclick="window.openFullscreenPDF('${data.fileUrl}', ${data.allowDownload !== false})" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2 shadow-lg"><i class="fas fa-book-reader"></i>وضع القراءة</button>` : ''}
                        <button onclick="window.shareLesson('${sectionId}', '${data.title}')" class="bg-green-100 hover:bg-green-200 text-green-700 p-2 rounded-xl transition" title="مشاركة على واتساب">
                            <i class="fab fa-whatsapp text-lg"></i>
                        </button>
                    </div>
                </div>
                <div class="prose dark:prose-invert max-w-none mb-10 text-lg leading-relaxed text-gray-700 dark:text-gray-300">${data.content || ''}</div>
            ...
            htmlContent += `
                <div class="mt-10 p-6 bg-amber-50 dark:bg-amber-900/20 border-2 border-amber-200 dark:border-amber-800/50 rounded-3xl relative group transition hover:shadow-lg hover:border-amber-300">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <span id="save-status-${subId}" class="text-[10px] font-bold px-3 py-1 rounded-full bg-white dark:bg-gray-800 shadow-sm opacity-0 transition-opacity"></span>
                    </div>
                    <textarea id="note-${subId}" class="w-full h-32 p-4 bg-white/60 dark:bg-gray-900/60 rounded-2xl border-none focus:ring-4 focus:ring-amber-200 dark:focus:ring-amber-900/50 text-gray-800 dark:text-white placeholder-amber-800/30 dark:placeholder-amber-200/30 resize-y transition shadow-inner" placeholder="سجّل ملاحظاتك هنا.. (يتم الحفظ تلقائياً)"></textarea>
                </div>
                <div class="mt-12 pt-8 border-t-2 border-dashed border-gray-100 dark:border-gray-700">
                    <h4 class="font-black text-gray-700 dark:text-gray-300 flex items-center gap-2 mb-6 text-lg"><i class="fas fa-comments text-blue-500"></i> ساحة النقاش</h4>
                    <div id="comments-list-${subId}" class="space-y-3 mb-6 max-h-[500px] overflow-y-auto custom-scrollbar p-2 bg-gray-50/50 dark:bg-gray-900/30 rounded-2xl"></div>
                    <div class="flex gap-3 items-center bg-white dark:bg-gray-700 p-2 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-600">
                        <img src="${user?.photoURL || 'https://via.pui-avatars.com/api/?background=random&name=Userlaceholder.com/40'}" class="w-10 h-10 rounded-full border-2 border-blue-100 shadow-sm">
                        ...
                        <button id="btn-comment-${subId}" class="w-12 h-12 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition shadow-lg flex items-center justify-center"><i class="fas fa-paper-plane"></i></button>
                    </div>
                </div>`;
            ...
            setTimeout(async () => {
                ...
                noteArea.addEventListener('input', () => {
                    ...
                    statusSpan.className = 'text-[10px] font-bold text-amber-600 bg-white dark:bg-gray-800 px-3 py-1 rounded-full opacity-100';
                    clearTimeout(saveTimeout);
                    saveTimeout = setTimeout(async () => {
                        try {
                            await setDoc(noteRef, { text: noteArea.value, sectionId, updatedAt: new Date() }, { merge: true });
                            statusSpan.textContent = '✅ تم الحفظ';
                            statusSpan.className = 'text-[10px] font-bold text-green-600 bg-white dark:bg-gray-800 px-3 py-1 rounded-full opacity-100';
...
const setupCommentsSystem = (sectionId, subId) => {
    ...
    onSnapshot(q, (snap) => {
        list.innerHTML = '';
        if (snap.empty) { list.innerHTML = '<p class="text-center text-gray-400 text-xs py-4 italic">لا توجد تعليقات.</p>'; return; }
        snap.forEach(d => {
            const c = d.data();
            list.innerHTML += `<div class="flex gap-3 animate-fade-in group w-full"><img src="${c.userPhoto || 'https://via.pui-avatars.com/api/?background=random&name=Userlaceholder.com/40'}" class="w-8 h-8 rounded-full border border-blue-100 self-start shadow-sm"><div class="flex-grow min-w-0"><div class="bg-blue-50/50 dark:bg-gray-700/50 p-3 rounded-2xl rounded-tr-none"><div class="flex justify-between items-center mb-1"><span class="font-black text-xs text-blue-700 dark:text-blue-400">${c.userName}</span><span class="text-[9px] text-gray-400 font-mono">${c.createdAt?.toDate().toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}</span></div><p class="text-sm text-gray-700 dark:text-gray-200 break-words leading-snug">${c.text}</p></div></div></div>`;
        });
    });
...
const checkSectionProgress = async (userId, sectionId, totalSubsections, container) => {
    ...
    if (snap.exists() && snap.data().completed) {
        container.innerHTML = `<div class="bg-green-100 text-green-700 px-4 py-2 rounded-2xl font-bold text-xs shadow-sm flex items-center gap-2 border border-green-200"><i class="fas fa-check-circle text-lg"></i> تم إكمال المادة</div>`;
    } else {
        container.innerHTML = `<button id="mark-complete-btn" class="bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-400 px-5 py-2.5 rounded-2xl font-bold text-xs shadow-md hover:shadow-lg transition transform active:scale-95 border border-blue-100 dark:border-gray-600 flex items-center gap-2"><i class="far fa-circle"></i> تحديد كمكتمل</button>`;
...
export const loadAssignmentsForUser = async () => {
    ...
    list.innerHTML = '<div class="p-20 text-center"><i class="fas fa-spinner fa-spin text-4xl text-blue-500"></i></div>';
    const user = auth.currentUser; if (!user) return;
    ...
    for (const docSnap of snap.docs) {
        ...
        if (d.collegeId === userData.collegeId && (d.departmentId === 'all' || d.departmentId === userData.departmentId)) {
            ...
            let statusHTML = userSub ? (userSub.grade ? `<div class="bg-green-100 p-4 rounded-2xl text-center"><p class="text-xs text-green-600 font-bold">تم التصحيح</p><p class="text-3xl font-black text-green-700">${userSub.grade}%</p></div>` : `<div class="bg-blue-50 p-4 rounded-2xl text-center text-blue-600 font-black">في انتظار التصحيح</div>`) : `<div class="flex flex-col gap-2 w-full md:w-64"><input type="file" id="file-${assignId}" class="hidden"><button onclick="document.getElementById('file-${assignId}').click()" class="bg-white text-blue-600 py-3 rounded-xl font-bold border-2 border-dashed border-blue-200 hover:border-blue-500 transition text-sm">اختر ملف الحل</button><button id="upload-btn-${assignId}" class="bg-blue-600 text-white py-3 rounded-xl font-black shadow-lg hover:bg-blue-700 transition text-sm">تسليم الواجب</button></div>`;
            const div = document.createElement('div');
            div.className = "bg-white dark:bg-gray-800 p-6 md:p-8 rounded-[2rem] shadow-lg border-r-8 border-blue-500 mb-6 flex flex-col md:flex-row justify-between items-center gap-6 transition hover:scale-[1.01]";
            div.innerHTML = `<div class="flex-grow text-right w-full"><h3 class="text-2xl font-black dark:text-white mb-2">${d.title}</h3><p class="text-gray-500 dark:text-gray-400 text-sm mb-4 leading-relaxed">${d.description}</p><div class="flex flex-wrap gap-3 text-[10px] font-bold uppercase tracking-wider text-gray-400"><span class="bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-lg"><i class="far fa-clock"></i> ${d.deadline}</span><span class="bg-blue-50 dark:bg-blue-900/30 text-blue-500 px-3 py-1 rounded-lg">${d.sectionTitle}</span></div>${userSub && userSub.feedback ? `<div class="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 rounded-xl text-xs text-yellow-800 dark:text-yellow-500"><strong>📝 ملاحظة المدرس:</strong> ${userSub.feedback}</div>` : ''}</div>${statusHTML}`;
...
export const loadStudySections = async () => {
    ...
    list.innerHTML = '<div class="col-span-full p-20 text-center"><i class="fas fa-circle-notch fa-spin text-4xl text-blue-600"></i></div>';
    const user = auth.currentUser; if (!user) return;
    const userDoc = await getDoc(doc(db, "users", user.uid)); const userData = userDoc.exists() ? userDoc.data() : null;
    ...
    snap.forEach(d => {
        ...
        if (canView) {
            ...
            card.className = "group bg-white dark:bg-gray-800 p-8 rounded-[2rem] shadow-xl border-b-8 border-blue-600 hover:scale-[1.03] transition-all relative overflow-hidden";
            card.innerHTML = `
                <button onclick="event.stopPropagation(); window.toggleFavorite('section', '${d.id}', '${data.title}')" class="absolute top-4 left-4 z-20 w-10 h-10 rounded-full bg-white/80 dark:bg-gray-700/80 shadow-lg flex items-center justify-center hover:scale-110 transition" id="fav-section-${d.id}">
                    <i class="far fa-heart text-red-400 text-lg"></i>
                </button>
                <a href="#section/${d.id}" class="block">
                    <div class="absolute -top-10 -right-10 w-32 h-32 bg-blue-50 dark:bg-blue-900/30 rounded-full transition-all group-hover:scale-150"></div>
                    <div class="relative z-10">
                        <h3 class="text-2xl font-black dark:text-white mb-2 tracking-tight">${data.title}</h3>
                        <p class="text-xs text-blue-500 font-bold uppercase tracking-widest bg-blue-50 dark:bg-blue-900/20 px-3 py-1 rounded-full w-fit">مادة دراسية</p>
                    </div>
                    <div class="mt-8 flex justify-end">
                        <span class="w-12 h-12 bg-blue-600 text-white rounded-2xl flex items-center justify-center shadow-lg group-hover:rotate-12 transition-transform"><i class="fas fa-arrow-left"></i></span>
                    </div>
                </a>
            `;
...
export const openCmsView = async () => {
    ...
    const collegesWithDepts = UNIVERSITY_STRUCTURE.map(c => `
        <div class="college-group bg-white/10 rounded-xl p-3 mb-2">
            <label class="flex items-center gap-2 cursor-pointer font-bold mb-2">
                <input type="checkbox" name="target-college" value="${c.id}" class="w-4 h-4 accent-blue-500 college-checkbox" data-college="${c.id}">
            ...
            <div class="departments-list hidden pr-6 space-y-1" id="depts-${c.id}">
                <label class="flex items-center gap-2 cursor-pointer text-xs opacity-80 hover:opacity-100">
                    <input type="checkbox" name="target-dept-${c.id}" value="all" class="w-3 h-3 accent-green-500" checked>
                    <span>✓ كل الأقسام</span>
                </label>
                ${c.departments.map(d => `
                    <label class="flex items-center gap-2 cursor-pointer text-xs opacity-80 hover:opacity-100">
                        <input type="checkbox" name="target-dept-${c.id}" value="${d.id}" class="w-3 h-3 accent-blue-500 dept-checkbox">
                        <span>${d.name}</span>
                    </label>
                `).join('')}
            </div>
    ...
    document.getElementById('admin-view-content').innerHTML = `
        <div class="mb-10 bg-blue-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="flex items-center gap-4">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" id="all-colleges-check" class="w-5 h-5 accent-green-500" checked>
                ...
                <button id="add-section-btn" class="w-full md:w-auto bg-white text-blue-900 px-8 py-4 rounded-2xl font-black hover:bg-blue-50 transition shadow-lg">
        ...
        <div id="edit-section-modal" class="hidden fixed inset-0 bg-black/80 z-[200] flex items-center justify-center p-4 backdrop-blur-sm">
            <div class="bg-white dark:bg-gray-800 w-full max-w-2xl max-h-[90vh] rounded-3xl overflow-hidden shadow-2xl animate-scale-in flex flex-col">
                <div class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-6 flex justify-between items-center flex-shrink-0">
                    <div class="flex items-center gap-3">
                ...
                <div class="p-6 overflow-y-auto">
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">اسم المادة</label>
                        <input type="text" id="edit-section-title" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white dark:border-gray-600 font-bold">
                    </div>
                    <div class="mb-4">
                        <label class="flex items-center gap-2 cursor-pointer mb-3">
                            <input type="checkbox" id="edit-all-colleges" class="w-5 h-5 accent-green-500">
                            <span class="font-bold dark:text-white">📌 لكل الكليات والأقسام</span>
                        </label>
                        <div id="edit-colleges-list" class="max-h-64 overflow-y-auto custom-scrollbar p-3 bg-gray-50 dark:bg-gray-700 rounded-xl">
                            ${UNIVERSITY_STRUCTURE.map(c => `
                                <div class="edit-college-group bg-white dark:bg-gray-600 rounded-xl p-3 mb-2">
                                    <label class="flex items-center gap-2 cursor-pointer font-bold mb-2">
                                        <input type="checkbox" name="edit-target-college" value="${c.id}" class="w-4 h-4 accent-blue-500 edit-college-checkbox" data-college="${c.id}">
                                        <span class="text-sm dark:text-white">🎓 ${c.name}</span>
                                    </label>
                                    <div class="edit-departments-list hidden pr-6 space-y-1" id="edit-depts-${c.id}">
                                        <label class="flex items-center gap-2 cursor-pointer text-xs opacity-80 hover:opacity-100">
                                            <input type="checkbox" name="edit-target-dept-${c.id}" value="all" class="w-3 h-3 accent-green-500 edit-all-depts" checked>
                                            <span class="dark:text-gray-200">✓ كل الأقسام</span>
                                        </label>
                                        ${c.departments.map(d => `
                                            <label class="flex items-center gap-2 cursor-pointer text-xs opacity-80 hover:opacity-100">
                                                <input type="checkbox" name="edit-target-dept-${c.id}" value="${d.id}" class="w-3 h-3 accent-blue-500 edit-dept-checkbox">
                                                <span class="dark:text-gray-200">${d.name}</span>
                                            </label>
                                        `).join('')}
                                    </div>
                                </div>
                    ...
                    <button onclick="window.saveSectionEdit()" class="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-3 rounded-xl font-bold hover:shadow-lg transition">
...
window.cmsBrowse = (collegeId = null, deptId = null) => {
    ...
    // Breadcrumb محسن
    html += `<div class="col-span-full mb-4 flex items-center justify-between bg-white dark:bg-gray-800 p-3 px-4 rounded-xl shadow-sm border dark:border-gray-700">
        <div class="flex items-center gap-2 text-sm">
            <button onclick="window.cmsBrowse()" class="text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 px-2 py-1 rounded-lg font-bold flex items-center gap-1 transition">
    ...
    if (collegeId) {
        ...
        html += `<i class="fas fa-chevron-left text-gray-300 text-xs"></i>
            <button onclick="window.cmsBrowse('${collegeId}')" class="${deptId ? 'text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30' : 'text-gray-600 dark:text-gray-300'} px-2 py-1 rounded-lg font-bold transition">
    ...
    if (!collegeId) {
        ...
        html += `<div class="col-span-full mb-4 flex items-center justify-between">
            <span class="text-sm font-bold text-gray-400 dark:text-gray-500"><i class="fas fa-database text-blue-500 ml-1"></i> إجمالي المواد: ${totalSections}</span>
        </div>`;

        html += `
        <div class="col-span-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            ${window.cmsGeneralSections.length > 0 ? `
            <div onclick="window.cmsShowGeneral()" class="cms-folder-card bg-gradient-to-br from-emerald-600 to-blue-700 group">
            ...
            return `
                <div onclick="window.cmsBrowse('${college.id}')" class="cms-folder-card ${count > 0 ? 'bg-gradient-to-br from-blue-600 to-indigo-700' : 'bg-gradient-to-br from-gray-500 to-gray-600'} group">
                    <div class="w-14 h-14 bg-white/15 rounded-2xl flex items-center justify-center mb-3 group-hover:scale-110 transition">
        ...
        html += `<div class="col-span-full mb-3 flex items-center gap-2">
            <span class="text-sm font-bold text-gray-400 dark:text-gray-500"><i class="fas fa-university text-blue-500 ml-1"></i> ${totalCollegeSections} مادة في هذه الكلية</span>
        </div>`;
        ...
        <div class="col-span-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            ...
            return `
                <div onclick="window.cmsBrowse('${collegeId}', '${dept.id}')" class="cms-folder-card ${sections.length > 0 ? 'bg-gradient-to-br from-indigo-600 to-blue-700' : 'bg-gradient-to-br from-gray-500 to-gray-600'} group">
                    <div class="w-14 h-14 bg-white/15 rounded-2xl flex items-center justify-center mb-3 group-hover:scale-110 transition">
        ...
        html += `<div class="col-span-full mb-3 flex items-center gap-3">
            <div class="flex-1 relative">
                ...
                <input type="text" id="cms-search-input" placeholder="بحث في المواد..." class="w-full p-2.5 pr-9 border rounded-xl dark:bg-gray-700 dark:text-white dark:border-gray-600 text-sm outline-none focus:ring-2 focus:ring-blue-500" oninput="window.cmsFilterSections()">
            </div>
            <span class="text-sm font-bold text-gray-400 dark:text-gray-500 whitespace-nowrap">${sections.length} مادة</span>
        </div>`;

        if (sections.length === 0) {
            html += `<div class="col-span-full text-center py-16 bg-gray-50 dark:bg-gray-800 rounded-2xl border-2 border-dashed dark:border-gray-700">
                ...
                <p class="text-xs text-gray-400 mt-1">اضغط "إضافة مادة جديدة" بالأعلى لإنشاء مادة</p>
            </div>`;
        } else {
            html += `<div id="cms-sections-list-inner" class="col-span-full grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                return `
                <div class="cms-section-card flex justify-between items-center p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md hover:border-blue-400 transition group" data-title="${s.title}">
...
window.cmsShowGeneral = () => {
    ...
    let html = `<div class="col-span-full mb-4 flex items-center justify-between bg-white dark:bg-gray-800 p-3 px-4 rounded-xl shadow-sm border dark:border-gray-700">
        <div class="flex items-center gap-2 text-sm">
            <button onclick="window.cmsBrowse()" class="text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 px-2 py-1 rounded-lg font-bold flex items-center gap-1 transition">
                <i class="fas fa-home"></i> الرئيسية
            </button>
        ...
        <span class="text-sm font-bold text-gray-400">${window.cmsGeneralSections.length} مادة</span>
    </div>`;
    ...
    html += `<div class="col-span-full mb-3">
        <div class="relative">
            <i class="fas fa-search absolute top-1/2 -translate-y-1/2 right-3 text-gray-400 text-sm"></i>
            <input type="text" id="cms-search-input" placeholder="بحث في المواد العامة..." class="w-full p-2.5 pr-9 border rounded-xl dark:bg-gray-700 dark:text-white dark:border-gray-600 text-sm outline-none focus:ring-2 focus:ring-blue-500" oninput="window.cmsFilterSections()">
        </div>
    </div>`;
    ...
    html += `<div id="cms-sections-list-inner" class="col-span-full grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        const createdAt = s.createdAt?.toDate ? s.createdAt.toDate().toLocaleDateString('ar-EG', { year: 'numeric', month: 'short', day: 'numeric' }) : '';
        return `
        <div class="cms-section-card flex justify-between items-center p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md hover:border-emerald-400 transition group" data-title="${s.title}">
            <div class="flex items-center gap-3 flex-1 min-w-0">
...
export const openAdminSubsectionEditor = async (sid) => {
    ...
    document.getElementById('admin-view-content').innerHTML = `
        <div class="mb-10 bg-white dark:bg-gray-800 p-8 rounded-[2.5rem] shadow-2xl border-t-8 border-blue-600">
            ...
            <input type="text" id="sub-title" placeholder="عنوان الدرس" class="w-full p-4 rounded-2xl border dark:bg-gray-700 dark:border-gray-600 dark:text-white mb-4 outline-none focus:ring-2 focus:ring-blue-500 font-bold">
            
            <div class="flex flex-wrap gap-4 mb-4">
                <label class="flex items-center gap-2 font-bold dark:text-white cursor-pointer bg-gray-50 dark:bg-gray-700 px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-600 hover:bg-blue-50 transition">
                    <input type="checkbox" id="is-book" class="w-5 h-5 accent-blue-600"> وضع الكتاب (Book Mode)
                </label>
                <label class="flex items-center gap-2 font-bold dark:text-white cursor-pointer bg-green-50 dark:bg-green-900/30 px-4 py-2 rounded-xl border border-green-200 dark:border-green-700 hover:bg-green-100 transition">
                    <input type="checkbox" id="allow-download" class="w-5 h-5 accent-green-600" checked> 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <div>
                    <label class="block text-xs font-black text-gray-400 mb-2 uppercase">رابط فيديو (YouTube) أو ملف خارجي:</label>
                    <input type="text" id="sub-url" class="w-full p-4 rounded-2xl border dark:bg-gray-700 dark:text-white outline-none" placeholder="https://...">
                </div>
                <div>
                    <label class="block text-xs font-black text-gray-400 mb-2 uppercase">أو رفع ملف مباشر (PDF/Image):</label>
                    <input type="file" id="sub-file" class="w-full p-3 border rounded-2xl dark:bg-gray-700 text-gray-500">
                </div>
            </div>
            
            <div class="flex gap-3">
                <button id="save-sub" class="flex-1 bg-blue-600 text-white px-10 py-4 rounded-2xl font-black shadow-lg hover:bg-blue-700 transition transform hover:-translate-y-1">حفظ الدرس</button>
                <button id="cancel-sub" class="hidden bg-gray-200 text-gray-600 px-6 py-4 rounded-2xl font-bold hover:bg-gray-300 transition">إلغاء</button>
            </div>
        </div>
...
const loadSubAdminList = async (sid) => {
    ...
    snap.forEach(d => {
        const data = d.data();
        const div = document.createElement('div');
        div.className = "flex justify-between items-center p-5 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700";
        const allowDownloadBadge = data.allowDownload !== false
            ? '<span class="bg-green-100 text-green-600 px-2 py-1 rounded text-xs"><i class="fas fa-download"></i></span>'
        ...
        div.innerHTML = `
            ...
            <div class="flex gap-2">
                <button class="bg-blue-50 text-blue-600 px-4 py-2 rounded-xl text-xs font-bold" onclick="window.editSubFull('${d.id}', '${data.title}', ${data.isBookMode}, '${data.fileUrl || ''}', ${data.allowDownload !== false})">✏️ تعديل</button>
...
export const openQuizCmsMain = async () => {
    ...
    content.innerHTML = `
        <div class="mb-8 p-6 bg-blue-50 dark:bg-gray-700 rounded shadow border-l-4 border-indigo-500">
            <h3 class="font-bold text-lg mb-4 dark:text-white">إضافة اختبار جديد</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <input type="text" id="quiz-title" placeholder="عنوان الاختبار" class="p-3 border rounded dark:bg-gray-800 dark:text-white">
                <input type="number" id="quiz-time" placeholder="الوقت (دقيقة)" class="p-3 border rounded dark:bg-gray-800 dark:text-white">
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <select id="quiz-col-select" class="p-3 border rounded dark:bg-gray-800 dark:text-white"><option value="">-- اختر الكلية --</option>${colOptions}</select>
                <select id="quiz-dept-select" class="p-3 border rounded dark:bg-gray-800 dark:text-white" disabled><option value="all">عام لكل الكلية</option></select>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-bold mb-1 dark:text-gray-300">المادة المرتبطة (اختياري):</label>
                <select id="quiz-section-select" class="w-full p-3 border rounded dark:bg-gray-800 dark:text-white" disabled><option value="general">نشاط عام (بدون مادة)</option></select>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <input type="number" id="quiz-attempts" placeholder="عدد المحاولات (0 = مفتوح)" class="p-3 border rounded dark:bg-gray-800 dark:text-white" value="1">
                <div class="flex items-center gap-2 p-3 bg-white dark:bg-gray-800 border rounded"><input type="checkbox" id="quiz-random" class="w-5 h-5"><label class="dark:text-gray-200">أسئلة عشوائية</label></div>
            </div>
            <textarea id="quiz-desc" placeholder="وصف الاختبار / تعليمات" class="w-full p-3 border rounded mb-4 dark:bg-gray-800 dark:text-white"></textarea>
            <button id="add-quiz-btn" class="bg-indigo-600 text-white px-6 py-2 rounded font-bold hover:bg-indigo-700 w-full md:w-auto">إنشاء الاختبار</button>
        </div>
...
const loadAdminQuizzes = async () => {
    ...
    list.innerHTML = '<div class="text-center p-4"><i class="fas fa-spinner fa-spin text-blue-600"></i> جاري التحميل...</div>';
    const q = query(collection(db, "quizzes"), orderBy("createdAt", "desc"));
    const snap = await getDocs(q);
    list.innerHTML = '';
    if (snap.empty) { list.innerHTML = '<p class="text-center text-gray-500">لا توجد اختبارات.</p>'; return; }
    snap.forEach(d => {
        const data = d.data();
        const div = document.createElement('div');
        div.className = "flex flex-col md:flex-row justify-between items-center p-4 bg-white dark:bg-gray-800 rounded shadow mb-3";
        div.innerHTML = `<div class="mb-2 md:mb-0"><h4 class="font-bold text-lg dark:text-white">${data.title}</h4><p class="text-xs text-indigo-600 font-bold mb-1">${data.sectionTitle || 'عام'} | ${data.departmentId}</p><p class="text-xs text-gray-500">${data.timeLimit} دقيقة</p></div><div class="flex gap-2"><button class="mng-questions-btn bg-blue-100 text-blue-700 px-4 py-1 rounded font-bold text-sm" onclick="window.openQuizQuestionsEditor('${d.id}', '${data.title}')">إدارة الأسئلة</button><button class="del-quiz-btn bg-red-100 text-red-700 px-4 py-1 rounded font-bold text-sm" onclick="window.deleteQuiz('${d.id}')">حذف</button></div>`;
        list.appendChild(div);
    });
};
...
window.openQuizQuestionsEditor = async (quizId, quizTitle) => {
    const content = document.getElementById('admin-view-content');
    content.innerHTML = `<div class="flex justify-between items-center mb-6"><h3 class="text-xl font-bold dark:text-white">أسئلة: <span class="text-indigo-600">${quizTitle}</span></h3><button onclick="window.openQuizCmsMain()" class="text-sm bg-gray-200 px-3 py-1 rounded">عودة</button></div><div class="mb-8 p-4 bg-gray-50 dark:bg-gray-800 border rounded"><h4 class="font-bold mb-3 dark:text-white">إضافة سؤال</h4><input type="text" id="q-text" placeholder="نص السؤال" class="w-full p-2 mb-3 border rounded dark:bg-gray-700 dark:text-white"><div class="grid grid-cols-2 gap-2 mb-3"><input type="text" id="opt-1" placeholder="الخيار 1" class="p-2 border rounded dark:bg-gray-700 dark:text-white"><input type="text" id="opt-2" placeholder="الخيار 2" class="p-2 border rounded dark:bg-gray-700 dark:text-white"><input type="text" id="opt-3" placeholder="الخيار 3" class="p-2 border rounded dark:bg-gray-700 dark:text-white"><input type="text" id="opt-4" placeholder="الخيار 4" class="p-2 border rounded dark:bg-gray-700 dark:text-white"></div><input type="text" id="correct-ans" placeholder="الإجابة الصحيحة" class="w-full p-2 mb-3 border rounded bg-green-50 dark:bg-green-900 dark:text-white"><button id="add-q-btn" class="w-full bg-blue-600 text-white py-2 rounded font-bold hover:bg-blue-700">إضافة السؤال</button></div><div id="questions-list" class="space-y-3"></div>`;
    ...
    snap.forEach((d, idx) => { const q = d.data(); list.innerHTML += `<div class="bg-white dark:bg-gray-700 p-3 rounded shadow mb-2"><div class="flex justify-between"><span class="font-bold text-blue-600 dark:text-blue-400">س${idx + 1}: ${q.text}</span><button class="text-red-500 font-bold" onclick="window.deleteQuestion('${quizId}', '${d.id}', '${quizTitle}')">×</button></div><p class="text-xs text-gray-500 mt-1">الإجابة: ${q.correctAnswer}</p></div>`; });
};
