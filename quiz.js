    ...
    if (container) {
        container.innerHTML = `
            <div class="max-w-md mx-auto bg-red-50 dark:bg-red-900/20 p-10 rounded-2xl shadow-2xl text-center mt-10 border-t-8 border-red-500">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 mb-6 text-right">
                ...
                <button onclick="window.location.hash='quiz'; window.location.reload();" 
                    class="w-full bg-gray-600 text-white px-6 py-3 rounded-xl font-bold">
...
export const loadQuizQuestionsForUser = async () => {
    ...
    container.innerHTML = `<div class="text-center p-10 col-span-full"><i class="fas fa-spinner fa-spin text-3xl text-indigo-600"></i><p class="mt-2 text-gray-500">${window.t?.('searching-quizzes') || 'جاري البحث عن اختبارات كليتك...'}</p></div>`;
    ...
    if (!userData || !userData.collegeId) {
        container.innerHTML = `<div class="col-span-full text-center p-10 bg-white dark:bg-gray-800 rounded-xl shadow"><i class="fas fa-exclamation-triangle text-3xl text-yellow-500 mb-3"></i><p class="font-bold dark:text-white">${window.t?.('update-college-first') || 'يرجى تحديث بيانات كليتك في الملف الشخصي أولاً.'}</p></div>`;
        return;
    }
    ...
    try {
        ...
        container.innerHTML = '';
        if (snap.empty) {
            container.innerHTML = `<div class="col-span-full text-center text-gray-500 p-10">${window.t?.('no-quizzes-available') || 'لا توجد اختبارات متاحة حالياً.'}</div>`;
            return;
        }
        ...
        for (const d of snap.docs) {
            ...
            if (matchCollege && matchDept) {
                ...
                div.className = `bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border-r-4 ${isLocked ? 'border-gray-400 opacity-75' : 'border-indigo-500'} mb-4 transform transition hover:scale-[1.01] flex flex-col justify-between`;
                ...
                div.innerHTML = `
                    <div>
                        <div class="flex justify-between items-start mb-2">
                            <div>
                                ...
                                <span class="text-[10px] bg-blue-100 text-indigo-700 px-2 py-1 rounded dark:bg-indigo-900/30 dark:text-blue-300 font-bold">${quiz.sectionTitle || (window.t?.('general-activity') || 'نشاط عام')}</span>
                            </div>
                            <div class="text-left min-w-[50px]">
                                ${previousScore !== null ? `<span class="block ${previousScore >= 50 ? 'text-green-600' : 'text-red-600'} font-bold text-lg">${previousScore}%</span>` : ''}
                            </div>
                        </div>
                        <p class="text-gray-600 dark:text-gray-300 mt-2 text-sm line-clamp-2 h-10">${quiz.description || (window.t?.('no-description') || 'لا يوجد وصف.')}</p>
                        <div class="flex items-center gap-4 mt-4 text-xs text-gray-500 dark:text-gray-400 border-t dark:border-gray-700 pt-3">
                    ...
                    <button id="btn-start-${d.id}" class="mt-4 w-full py-2 rounded-lg font-bold transition shadow-sm flex items-center justify-center gap-2 ${isLocked ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-indigo-600 text-white hover:bg-indigo-700'}" ${isLocked ? 'disabled' : ''}>
        ...
        if (!hasQuizzes) {
            container.innerHTML = `<div class="col-span-full flex flex-col items-center justify-center text-center p-10 bg-white dark:bg-gray-800 rounded-xl"><i class="fas fa-clipboard-list text-6xl text-gray-200 mb-4"></i><p class="text-gray-500 font-bold">${window.t?.('no-quizzes-for-dept') || 'لا توجد اختبارات متاحة لقسمك'} (${userData.departmentId}).</p></div>`;
        }
    } catch (e) {
        console.error(e);
...
const startQuiz = async (quizId, quizData) => {
    ...
    container.innerHTML = `
        <div class="max-w-lg mx-auto bg-yellow-50 dark:bg-yellow-900/20 p-8 rounded-2xl shadow-xl text-center border-2 border-yellow-400">
            ...
            <ul class="text-right text-gray-600 dark:text-gray-300 space-y-3 mb-6">
            ...
            <button id="start-quiz-btn" class="w-full bg-gradient-to-r from-indigo-600 to-blue-600 text-white px-8 py-4 rounded-xl font-bold text-lg hover:from-indigo-700 hover:to-blue-700 shadow-lg transition">
    ...
    document.getElementById('start-quiz-btn').onclick = async () => {
        ...
        container.innerHTML = '<div class="text-center p-20"><i class="fas fa-spinner fa-spin text-4xl text-indigo-600"></i><p class="mt-4 text-lg dark:text-white">جاري تجهيز ورقة الامتحان...</p></div>';
        ...
        try {
            ...
            const renderQuestion = () => {
                ...
                const questionNumbers = questions.map((_, idx) => {
                    ...
                    return `<button onclick="window.goToQ(${idx})" 
                        ...
                        ${isCurrent ? 'bg-indigo-600 text-white ring-2 ring-blue-300' :
                            isAnswered ? 'bg-green-500 text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-600 dark:text-gray-300'}"
                    >${idx + 1}</button>`;
                }).join('');

                container.innerHTML = `
                <div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-700 animate-fade-in">
                    <div class="bg-gradient-to-r from-indigo-600 to-blue-600 text-white p-4 md:p-6 shadow-md">
                        <div class="flex justify-between items-center mb-4">
                            <div><h3 class="font-bold text-xl">${quizData.title}</h3><p class="text-blue-200 text-xs mt-1">السؤال ${currentQIndex + 1} من ${questions.length}</p></div>
                    ...
                    <div class="p-6 md:p-10">
                        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mb-8"><div class="bg-indigo-500 h-2 rounded-full transition-all duration-500" style="width: ${((currentQIndex + 1) / questions.length) * 100}%"></div></div>
                    ...
                    return `
                                <label class="flex items-center p-4 border-2 rounded-xl cursor-pointer hover:bg-blue-50 dark:hover:bg-gray-700 transition-all duration-200 group ${isSelected ? 'border-indigo-500 bg-blue-50 dark:bg-indigo-900/20 ring-1 ring-indigo-500' : 'border-gray-200 dark:border-gray-600'}">
                                    <div class="relative flex items-center justify-center w-6 h-6 rounded-full border-2 border-gray-300 group-hover:border-indigo-500 mr-3 ml-4 transition-colors flex-shrink-0">
                                        ${isSelected ? '<div class="w-3 h-3 bg-indigo-600 rounded-full"></div>' : ''}
                                    ...
                                    <span class="text-base md:text-lg text-gray-700 dark:text-gray-200 font-medium select-none leading-relaxed">${displayOpt}</span>
                                </label>`;
                }).join('')}
                        </div>
                    </div>
                    <div class="p-4 md:p-6 bg-gray-50 dark:bg-gray-900 border-t dark:border-gray-700 flex flex-wrap justify-between items-center gap-3">
                        <button onclick="window.prevQ()" class="px-4 md:px-6 py-2 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 font-bold transition text-sm md:text-base ${currentQIndex === 0 ? 'invisible' : ''}"><i class="fas fa-arrow-right ml-1 md:ml-2"></i> السابق</button>
                        ${currentQIndex === questions.length - 1
                        ? `<button onclick="window.submitQuizNow()" class="flex-1 md:flex-none px-6 md:px-8 py-3 md:py-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl font-black text-base md:text-lg hover:from-green-700 hover:to-emerald-700 shadow-xl transition transform hover:-translate-y-1 animate-pulse">تسليم الإجابة <i class="fas fa-check mr-2"></i></button>`
                        : `<button onclick="window.nextQ()" class="px-6 md:px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 shadow-lg transition transform hover:-translate-y-1">التالي <i class="fas fa-arrow-left mr-2"></i></button>`
                    }
                    </div>
                </div>`;
                updateTimerDisplay();
            ...
            window.submitQuizNow = async () => {
                ...
                container.innerHTML = '<div class="text-center p-20"><div class="animate-spin rounded-full h-16 w-16 border-t-4 border-b-4 border-green-500 mx-auto mb-4"></div><h2 class="text-2xl font-bold dark:text-white">جاري تصحيح الإجابات...</h2></div>';
                ...
                try {
                    ...
                    let answersReviewHtml = questions.map((q, idx) => {
                        ...
                        return `
                            <div class="p-3 rounded-xl ${isCorrect ? 'bg-green-50 dark:bg-green-900/20 border-r-4 border-green-500' : 'bg-red-50 dark:bg-red-900/20 border-r-4 border-red-500'}">
                                <div class="flex justify-between items-start mb-2">
                                    <span class="font-bold text-sm ${isCorrect ? 'text-green-600' : 'text-red-600'}">${isCorrect ? '✅' : '❌'} س${idx + 1}</span>
                                </div>
                                <p class="text-sm text-gray-700 dark:text-gray-300 mb-2">${q.text}</p>
                                <div class="text-xs space-y-1">
                                    <p class="text-gray-500">إجابتك: <span class="${isCorrect ? 'text-green-600 font-bold' : 'text-red-600 font-bold'}">${userAnswer}</span></p>
                                    ${!isCorrect ? `<p class="text-green-600">الإجابة الصحيحة: <span class="font-bold">${q.correctAnswer}</span></p>` : ''}
                                </div>
                            </div>
                        `;
                    }).join('');
                    ...
                    container.innerHTML = `
                    <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden mt-10 animate-fade-in">
                        <div class="p-8 text-center border-b-8 ${score >= 50 ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : 'border-red-500 bg-red-50 dark:bg-red-900/20'}">
                            <div class="text-7xl mb-4">${score >= 90 ? '🏆' : (score >= 50 ? '✅' : '💪')}</div>
                            <h2 class="text-4xl font-black ${score >= 50 ? 'text-green-600' : 'text-red-600'}">${score}%</h2>
                            <p class="text-gray-600 dark:text-gray-300 mt-2">${correctCount} صحيح من ${questions.length} سؤال</p>
                        </div>
                        
                        <div class="p-6">
                            <h3 class="font-bold text-lg dark:text-white mb-4"><i class="fas fa-list-check text-indigo-500"></i> مراجعة الإجابات</h3>
                        ...
                        <div class="p-6 bg-gray-50 dark:bg-gray-900">
                            <button onclick="window.location.hash='quiz'; window.location.reload();" 
                                class="w-full bg-blue-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-700 shadow-lg transition">
                    ...
                    container.innerHTML = `
                        <div class="max-w-md mx-auto text-center p-10 bg-white dark:bg-gray-800 rounded-2xl shadow-xl mt-10">
                            <div class="text-5xl mb-4">⚠️</div>
                            <h2 class="text-xl font-bold text-red-600 mb-2">حدث خطأ في الحفظ</h2>
                            <p class="text-gray-600 dark:text-gray-300 mb-2">نتيجتك: <span class="font-black text-2xl ${score >= 50 ? 'text-green-600' : 'text-red-600'}">${score}%</span></p>
                            <p class="text-gray-500 text-sm mb-4">${correctCount} صحيح من ${questions.length}</p>
                            <p class="text-xs text-red-400 mb-4">${e.message || 'خطأ غير معروف'}</p>
                            <button onclick="window.location.hash='quiz'; window.location.reload();" class="w-full bg-gray-600 text-white px-4 py-3 rounded-xl font-bold">العودة</button>
                        </div>`;
                }
            };
            renderQuestion();
...
export const openQuizCmsMain = async () => {
    ...
    document.getElementById('admin-view-content').innerHTML = `
        <div class="mb-10 bg-white dark:bg-gray-800 p-8 rounded-[2.5rem] shadow-xl border-t-8 border-indigo-600 animate-fade-in">
            <h3 class="font-black text-2xl mb-6 dark:text-white flex items-center gap-2"><i class="fas fa-plus-circle text-indigo-600"></i> إنشاء اختبار جديد</h3>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <input type="text" id="new-quiz-title" placeholder="عنوان الاختبار (مثال: كويز فيزياء 1)" class="p-4 border rounded-2xl dark:bg-gray-700 dark:text-white font-bold outline-none">
                <input type="number" id="new-quiz-time" placeholder="المدة بالدقائق" class="p-4 border rounded-2xl dark:bg-gray-700 dark:text-white font-bold outline-none">
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <select id="admin-quiz-col" class="p-4 border rounded-2xl dark:bg-gray-700 dark:text-white font-bold outline-none"><option value="">-- اختر الكلية --</option>${colOptions}</select>
                <select id="admin-quiz-dept" class="p-4 border rounded-2xl dark:bg-gray-700 dark:text-white font-bold outline-none" disabled><option value="all">كل الأقسام</option></select>
            </div>

            <div class="flex gap-4 mb-6">
                <label class="flex items-center gap-2 cursor-pointer bg-gray-50 dark:bg-gray-700 px-4 py-2 rounded-xl border dark:border-gray-600">
                    <input type="checkbox" id="quiz-random" class="w-5 h-5 accent-indigo-600"> <span class="dark:text-white font-bold text-sm">ترتيب عشوائي للأسئلة</span>
                </label>
                <input type="number" id="quiz-attempts" placeholder="المحاولات (0 = لا نهائي)" class="w-40 p-2 border rounded-xl dark:bg-gray-700 dark:text-white text-center font-bold text-sm">
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button id="create-quiz-btn" class="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black hover:bg-indigo-700 transition shadow-lg transform hover:-translate-y-1"><i class="fas fa-plus ml-2"></i>إنشاء اختبار يدوي</button>
                <button id="ai-quiz-btn" class="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-4 rounded-2xl font-black hover:from-blue-700 hover:to-indigo-700 transition shadow-lg transform hover:-translate-y-1"><i class="fas fa-robot ml-2"></i>توليد بالذكاء الاصطناعي 🤖</button>
            </div>
        </div>
        ...
        <div class="flex justify-between items-center mb-4 px-2">
            ...
            <div class="flex gap-2">
                ...
                <button onclick="window.openQuizResults()" class="bg-gradient-to-r from-green-600 to-emerald-600 text-white px-5 py-2 rounded-xl font-bold hover:from-green-700 hover:to-emerald-700 transition flex items-center gap-2 shadow-lg">
        ...
        <div id="questions-modal" class="hidden fixed inset-0 bg-black/80 z-[200] flex items-center justify-center p-4 backdrop-blur-sm">
            <div class="bg-white dark:bg-gray-800 w-full max-w-4xl h-[90vh] rounded-[2rem] flex flex-col relative shadow-2xl border border-gray-700">
                <div class="p-6 border-b dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900 rounded-t-[2rem]">
                    <h2 class="text-2xl font-black dark:text-white flex items-center gap-2"><i class="fas fa-list-ol text-indigo-600"></i> أسئلة الاختبار</h2>
                ...
                <div class="flex-1 overflow-y-auto p-6 bg-gray-100 dark:bg-gray-800/50 custom-scrollbar">
                    <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm mb-6 border border-gray-200 dark:border-gray-700">
                        <h4 class="font-bold mb-4 dark:text-white border-b pb-2">إضافة سؤال جديد</h4>
                        <textarea id="q-text" class="w-full p-3 border rounded-xl mb-3 dark:bg-gray-700 dark:text-white font-bold" placeholder="نص السؤال..."></textarea>
                        <div class="grid grid-cols-2 gap-3 mb-3">
                            <input type="text" id="opt-1" placeholder="الخيار الأول" class="p-2 border rounded-lg dark:bg-gray-700 dark:text-white">
                            <input type="text" id="opt-2" placeholder="الخيار الثاني" class="p-2 border rounded-lg dark:bg-gray-700 dark:text-white">
                            <input type="text" id="opt-3" placeholder="الخيار الثالث" class="p-2 border rounded-lg dark:bg-gray-700 dark:text-white">
                            <input type="text" id="opt-4" placeholder="الخيار الرابع" class="p-2 border rounded-lg dark:bg-gray-700 dark:text-white">
                        </div>
                        <div class="mb-4">
                            <label class="text-xs font-bold text-gray-500 uppercase">الإجابة الصحيحة:</label>
                            <select id="correct-opt" class="w-full p-2 border rounded-lg dark:bg-gray-700 dark:text-white font-bold">
                        ...
                        <button id="add-q-btn" class="w-full bg-green-600 text-white py-3 rounded-xl font-bold hover:bg-green-700 transition">إضافة السؤال للقائمة</button>
                    </div>
    ...
    snap.forEach(d => {
        const q = d.data();
        list.innerHTML += `
            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border-l-8 border-indigo-500 flex justify-between items-center group hover:shadow-lg transition">
                <div>
                    <h4 class="font-black text-lg dark:text-white">${q.title}</h4>
                    <p class="text-xs text-gray-500 font-bold mt-1 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded w-fit">${q.collegeId} / ${q.departmentId}</p>
                </div>
                <div class="flex gap-2">
                    ...
                    <button onclick="window.openQuizResults('${d.id}', '${q.title.replace(/'/g, "\\'")}')" class="bg-green-50 text-green-600 w-10 h-10 rounded-xl hover:bg-green-600 hover:text-white transition flex items-center justify-center" title="النتائج"><i class="fas fa-chart-bar"></i></button>
                    <button onclick="window.manageQuestions('${d.id}')" class="bg-blue-50 text-blue-600 w-10 h-10 rounded-xl hover:bg-blue-600 hover:text-white transition flex items-center justify-center" title="الأسئلة"><i class="fas fa-list"></i></button>
...
export const openQuizResultsDashboard = async (quizId = null, quizTitle = '') => {
    ...
    document.getElementById('admin-view-content').innerHTML = `
        <!-- Filters -->
        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg mb-6 border-t-4 border-green-500">
            <h3 class="font-black text-lg mb-4 dark:text-white flex items-center gap-2"><i class="fas fa-filter text-green-500"></i> فلترة النتائج</h3>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <select id="filter-quiz" class="p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-bold">
                    ${quizOptions}
                </select>
                <select id="filter-college" class="p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-bold">
                    <option value="">كل الكليات</option>
                    ${colOptions}
                </select>
                <select id="filter-dept" class="p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-bold" disabled>
                    <option value="">كل الأقسام</option>
                </select>
                <button id="apply-filter-btn" class="bg-green-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-green-700 transition flex items-center justify-center gap-2">
        ...
        <div class="flex justify-between items-center mb-4 flex-wrap gap-3">
            ...
            <div class="flex gap-2">
                ...
                <button id="export-excel-btn" class="bg-gradient-to-r from-green-600 to-emerald-600 text-white px-5 py-2 rounded-xl font-bold hover:from-green-700 hover:to-emerald-700 transition flex items-center gap-2 shadow-lg">
            ...
            <div class="overflow-x-auto">
                <table class="w-full" id="results-table">
                    <thead class="bg-gray-100 dark:bg-gray-700">
                        <tr>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">#</th>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">الطالب</th>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">الاختبار</th>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">المحاولة</th>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">الدرجة</th>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">الصحيحة</th>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">التاريخ</th>
                        </tr>
                    </thead>
                    <tbody id="results-tbody" class="divide-y dark:divide-gray-700"></tbody>
                </table>
            </div>
            <div id="results-loading" class="p-10 text-center">
                <i class="fas fa-spinner fa-spin text-3xl text-green-500"></i>
                <p class="mt-2 text-gray-500">جاري تحميل النتائج...</p>
            </div>
            <div id="results-empty" class="hidden p-10 text-center">
                <i class="fas fa-inbox text-5xl text-gray-300 mb-4"></i>
                <p class="text-gray-500 font-bold">لا توجد نتائج</p>
            </div>
        </div>
    `;
    ...
    const loadResults = async () => {
        ...
        try {
            ...
            results.slice(0, 100).forEach((r, idx) => {
                ...
                const scoreClass = r.score >= 50 ? 'text-green-600 bg-green-100' : 'text-red-600 bg-red-100';
                ...
                tbody.innerHTML += `
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition cursor-pointer" onclick="window.location.hash='profile/${r.userId}'">
                        <td class="p-4 font-bold text-gray-500">${idx + 1}</td>
                        <td class="p-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <span class="font-bold dark:text-white hover:text-indigo-600 transition">${r.userName || 'مجهول'}</span>
                                    <span class="block text-xs text-gray-400">${r.userId?.slice(0, 8) || ''}...</span>
                                </div>
                            </div>
                        </td>
                        <td class="p-4 font-bold text-indigo-600 dark:text-indigo-400">${r.quizTitle || 'N/A'}</td>
                        <td class="p-4">
                            <span class="bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-1 rounded-lg font-bold text-sm">${attemptNum} / ${totalAttempts}</span>
                        </td>
                        <td class="p-4"><span class="px-3 py-1 rounded-full font-black ${scoreClass}">${r.score}%</span></td>
                        <td class="p-4 text-gray-600 dark:text-gray-300">${r.correct || 0} / ${r.questionsCount || '?'}</td>
    ...
    document.getElementById('reset-grades-btn').onclick = async () => {
        ...
        confirmModal.innerHTML = `
            <div class="bg-white dark:bg-gray-800 w-full max-w-md rounded-2xl overflow-hidden shadow-2xl">
                ...
                <div class="p-6 text-center">
                    ...
                    <input type="text" id="confirm-text" placeholder="اكتب 'تأكيد' للمتابعة" class="w-full p-3 border-2 border-red-300 rounded-xl text-center font-bold dark:bg-gray-700 dark:text-white focus:border-red-500 outline-none">
                </div>
                <div class="p-4 bg-gray-50 dark:bg-gray-900 flex gap-3">
                    <button id="cancel-reset-btn" class="flex-1 bg-gray-500 text-white py-3 rounded-xl font-bold hover:bg-gray-600 transition">إلغاء</button>
                ...
                <table class="w-full" id="cheating-table">
                    <thead class="bg-gray-100 dark:bg-gray-700">
                        <tr>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">#</th>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">الطالب</th>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">الكويز</th>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">المخالفات</th>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">السبب</th>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">التاريخ</th>
                            <th class="p-4 text-right font-black text-gray-700 dark:text-gray-200">التفاصيل</th>
                        </tr>
                    </thead>
                    <tbody id="cheating-tbody" class="divide-y dark:divide-gray-700"></tbody>
                </table>
            </div>
            
            <div id="cheating-loading" class="p-10 text-center">
                <i class="fas fa-spinner fa-spin text-3xl text-red-500"></i>
                <p class="mt-2 text-gray-500">جاري تحميل السجلات...</p>
            </div>
            
            <div id="cheating-empty" class="hidden p-10 text-center">
                <i class="fas fa-check-circle text-5xl text-green-400 mb-4"></i>
                <p class="text-gray-500 font-bold text-lg">لا توجد محاولات غش مسجلة 🎉</p>
            </div>
        </div>
    `;
        ...
        snap.forEach(d => {
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-red-50 dark:hover:bg-red-900/10 transition">
                    <td class="p-4 font-bold text-gray-500">${idx}</td>
                    <td class="p-4">
                        <span class="font-bold dark:text-white">${log.userName || 'مجهول'}</span>
                        <span class="block text-xs text-gray-400">${log.userId?.slice(0, 8) || ''}...</span>
                    </td>
                    <td class="p-4 text-indigo-600 dark:text-indigo-400 font-bold text-sm">${log.quizId?.slice(0, 10) || 'N/A'}...</td>
                    <td class="p-4">
                        <span class="bg-red-100 text-red-600 px-3 py-1 rounded-full font-black">${log.totalViolations || 0}</span>
                    </td>
                    <td class="p-4 text-sm text-gray-600 dark:text-gray-300 max-w-[200px] truncate">${log.reason || 'N/A'}</td>
                    <td class="p-4 text-gray-500 text-sm">${dateStr}</td>
                    <td class="p-4">
                        <button onclick="this.nextElementSibling.classList.toggle('hidden')" class="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-3 py-1 rounded-lg text-xs font-bold hover:bg-gray-200 dark:hover:bg-gray-600 transition">
                            <i class="fas fa-eye"></i> عرض
                        </button>
                        <div class="hidden absolute bg-white dark:bg-gray-800 border dark:border-gray-600 rounded-xl shadow-xl p-4 mt-2 z-50 max-w-xs">
                            <h5 class="font-bold text-red-500 mb-2 text-sm">📋 تفاصيل المخالفات:</h5>
                            <ul class="space-y-1 text-gray-600 dark:text-gray-300">${violationsList}</ul>
                        </div>
                    </td>
                </tr>
            `;
    ...
    modal.innerHTML = `
        <div class="bg-white dark:bg-gray-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div class="p-5 space-y-4">
                <div>
                    <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-1">📝 العنوان</label>
                    <input type="text" id="edit-quiz-title" value="${quiz.title || ''}" class="w-full p-3 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-yellow-500 outline-none">
                </div>
                
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-1">⏱️ الوقت (دقيقة)</label>
                        <input type="number" id="edit-quiz-time" value="${quiz.timeLimit || 15}" min="1" class="w-full p-3 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-yellow-500 outline-none text-center">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-1">🔄 المحاولات</label>
                        <input type="number" id="edit-quiz-attempts" value="${quiz.maxAttempts || 0}" min="0" class="w-full p-3 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-yellow-500 outline-none text-center" placeholder="0 = لا نهائي">
                    </div>
                </div>
                
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-1">🎓 الكلية</label>
                        <select id="edit-quiz-college" class="w-full p-3 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-yellow-500 outline-none">
                            <option value="all">كل الكليات</option>
                            ${colOptions}
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-1">📁 القسم</label>
                        <select id="edit-quiz-dept" class="w-full p-3 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-yellow-500 outline-none">
                            ${deptOptions}
                        </select>
                    </div>
                </div>
                
                <label class="flex items-center gap-3 cursor-pointer bg-gray-100 dark:bg-gray-700 p-3 rounded-xl border-2 border-gray-200 dark:border-gray-600">
                    <input type="checkbox" id="edit-quiz-random" class="w-5 h-5 accent-yellow-500" ${quiz.randomize ? 'checked' : ''}>
                    <span class="font-bold text-gray-700 dark:text-white">🎲 ترتيب عشوائي للأسئلة</span>
                </label>
...
window.manageQuestions = async (quizId) => {
    ...
    const loadQs = async () => {
        list.innerHTML = '<p class="text-center text-gray-400">جاري التحميل...</p>';
        const snap = await getDocs(collection(db, "quizzes", quizId, "questions"));
        list.innerHTML = '';
        if (snap.empty) list.innerHTML = '<p class="text-center text-gray-400 py-4">لا توجد أسئلة مضافة.</p>';

        snap.forEach((d, idx) => {
            const q = d.data();
            list.innerHTML += `
                <div class="bg-white dark:bg-gray-700 p-4 rounded-xl shadow-sm border dark:border-gray-600 flex justify-between gap-4">
                    <div>
                        <span class="font-bold text-indigo-600 text-xs">سؤال ${idx + 1}</span>
                        <p class="font-bold dark:text-white text-sm mt-1">${q.text}</p>
                        <p class="text-xs text-green-600 mt-1 font-bold">الإجابة: ${q.correctAnswer}</p>
                    </div>
...
export const openLeaderboardView = async () => {
    ...
    try {
        ...
        if (snap.empty) {
            list.innerHTML = '<p class="text-center text-gray-500 font-bold p-10">لا توجد بيانات كافية لعرض لوحة الشرف.</p>';
            return;
        }
        ...
        snap.forEach(d => {
            ...
            let rankStyle = "bg-white dark:bg-gray-800 border-l-4 border-gray-300";
            let rankIcon = `<span class="font-bold text-gray-500 text-xl w-8 text-center">#${rank}</span>`;
            ...
            if (rank === 1) {
                ...
                rankStyle = "bg-gradient-to-r from-gray-50 to-slate-50 dark:from-gray-700/50 dark:to-slate-700/50 border-l-4 border-gray-400 shadow-md";
                rankIcon = "🥈";
            } else if (rank === 3) {
                ...
                <div class="flex items-center gap-4 p-4 rounded-xl shadow-sm mb-3 ${rankStyle} transition-all hover:shadow-md animate-fade-in">
                    ...
                    <div class="flex-grow min-w-0">
                        <div class="flex flex-col md:flex-row md:items-center gap-2">
                            <h4 class="font-bold text-lg truncate dark:text-white cursor-pointer hover:text-blue-600 transition" onclick="window.openUserProfile?.('${d.id}')">${u.displayName}</h4>
                            ...
                            ${collegeShortName ? `<span class="text-[10px] bg-blue-50 text-blue-600 border border-blue-100 px-2 py-0.5 rounded-full truncate w-fit dark:bg-blue-900/40 dark:text-blue-200 dark:border-blue-800">${collegeShortName}</span>` : ''}
                        </div>
                        <div class="text-sm text-gray-500 dark:text-gray-400 flex flex-wrap gap-3 mt-1 items-center">
                            <span class="font-bold text-indigo-600 flex items-center gap-1"><i class="fas fa-star"></i> ${xp} XP</span>
                            <span class="font-bold text-orange-500 flex items-center gap-1"><i class="fas fa-fire"></i> ${u.currentStreak || u.streak || 0} يوم</span>
                            ${studyHours > 0 ? `<span class="font-bold text-blue-500 flex items-center gap-1"><i class="fas fa-clock"></i> ${studyHours} ساعة</span>` : ''}
...
window.openAIQuizGenerator = () => {
    ...
    modal.innerHTML = `
        <div class="bg-white dark:bg-gray-800 w-full max-w-2xl rounded-[2rem] shadow-2xl overflow-hidden my-4">
            <div class="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white">
                ...
                <p class="text-blue-100 text-sm mt-2">اكتب الموضوع وحدد عدد الأسئلة وسيقوم الـ AI بتوليد الكويز تلقائياً ✨</p>
            </div>
            
            <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                <div>
                    <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-2">📝 عنوان الكويز</label>
                    <input type="text" id="ai-quiz-title" class="w-full p-4 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-blue-500 outline-none transition" placeholder="مثال: اختبار الفيزياء - قوانين نيوتن">
                </div>
                
                <!-- اختيار الكلية والقسم -->
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-2">🎓 الكلية</label>
                        <select id="ai-quiz-college" class="w-full p-4 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-blue-500 outline-none">
                            <option value="all">كل الكليات</option>
                            ${colOptions}
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-2">📁 القسم</label>
                        <select id="ai-quiz-dept" class="w-full p-4 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-blue-500 outline-none">
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-2">📚 الموضوع والمحتوى (اكتب كل التفاصيل)</label>
                    <textarea id="ai-quiz-topic" class="w-full h-32 p-4 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-blue-500 outline-none transition resize-none" placeholder="اكتب هنا كل المعلومات والمفاهيم اللي عايز الأسئلة تتكون منها...
...
- قانون نيوتن الثالث (الفعل ورد الفعل)"></textarea>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-2">🔢 عدد الأسئلة</label>
                        <select id="ai-quiz-count" class="w-full p-4 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-blue-500 outline-none">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-2">📊 مستوى الصعوبة</label>
                        <select id="ai-quiz-difficulty" class="w-full p-4 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-blue-500 outline-none">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-2">⏱️ وقت الاختبار (دقيقة)</label>
                        <input type="number" id="ai-quiz-time" value="15" min="5" max="180" class="w-full p-4 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-blue-500 outline-none text-center">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-2">🔄 عدد المحاولات</label>
                        <input type="number" id="ai-quiz-attempts" value="1" min="0" max="10" class="w-full p-4 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-blue-500 outline-none text-center" placeholder="0 = لا نهائي">
                    </div>
                </div>
                
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-2">🌐 اللغة</label>
                        <select id="ai-quiz-lang" class="w-full p-4 border-2 rounded-xl dark:bg-gray-700 dark:text-white font-bold focus:border-blue-500 outline-none">
                            <option value="ar" selected>العربية 🇪🇬</option>
                            <option value="en">English 🇺🇸</option>
                        </select>
                    </div>
                    <div class="flex items-end">
                        <label class="flex items-center gap-3 cursor-pointer bg-gray-100 dark:bg-gray-700 p-4 rounded-xl border-2 border-gray-200 dark:border-gray-600 w-full hover:bg-blue-50 dark:hover:bg-blue-900/30 transition">
                            <input type="checkbox" id="ai-quiz-random" class="w-5 h-5 accent-blue-600" checked>
                            <span class="font-bold text-gray-700 dark:text-white">🎲 ترتيب عشوائي</span>
                        </label>
                    </div>
                </div>
                
                <div id="ai-quiz-status" class="hidden p-4 rounded-xl bg-blue-50 dark:bg-blue-900/30 text-center">
                    <i class="fas fa-spinner fa-spin text-2xl text-blue-600 mb-2"></i>
                    <p class="text-blue-700 dark:text-blue-300 font-bold">جاري توليد الأسئلة... انتظر قليلاً ⏳</p>
                </div>
                
                <button id="generate-ai-quiz-btn" onclick="window.generateAIQuiz()" class="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-4 rounded-2xl font-black hover:from-blue-700 hover:to-indigo-700 transition shadow-lg transform hover:-translate-y-1 flex items-center justify-center gap-2">
...
window.generateAIQuiz = async () => {
    ...
    statusDiv.innerHTML = `
        ...
        <p class="text-blue-700 dark:text-blue-300 font-bold">جاري توليد الأسئلة... انتظر قليلاً ⏳</p>
    `;
    statusDiv.className = 'p-4 rounded-xl bg-blue-50 dark:bg-blue-900/30 text-center';
    btn.disabled = true;
...
ملاحظة: correctIndex هو رقم الخيار الصحيح (0 = الأول، 1 = الثاني، 2 = الثالث، 3 = الرابع)`;
    ...
    try {
        ...
        statusDiv.innerHTML = `
            ...
            <p class="text-green-700 dark:text-green-300 font-bold">✅ تم توليد ${validQuestions.length} سؤال بنجاح!</p>
            <p class="text-sm text-gray-500 mt-1">الكلية: ${collegeId === 'all' ? 'الكل' : collegeId} | القسم: ${departmentId === 'all' ? 'الكل' : departmentId}</p>
        `;
        statusDiv.className = 'p-4 rounded-xl bg-green-50 dark:bg-green-900/30 text-center';

        btn.innerHTML = '<i class="fas fa-check"></i> تم بنجاح!';
        btn.className = 'w-full bg-green-600 text-white py-4 rounded-2xl font-black shadow-lg flex items-center justify-center gap-2';
        ...
        statusDiv.innerHTML = `
            ...
            <p class="text-xs text-gray-500 mt-2">حاول تقليل عدد الأسئلة أو تبسيط الموضوع</p>
        `;
