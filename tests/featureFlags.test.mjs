...
test('official mode keeps AI, streak and inspect protection enabled by default',()=>assert.deepEqual(getEnabledFeatures(OFFICIAL_MODE_DEFAULTS),['streak','aiAssistant','inspectProtection']));
...
test('inspect protection is locked by default but owner can disable it',()=>{assert.equal(isFeatureEnabled(OFFICIAL_MODE_DEFAULTS,'inspectProtection'),true);const flags=normalizeFeatureFlags({inspectProtection:false});assert.equal(isFeatureEnabled(flags,'inspectProtection'),false);});
