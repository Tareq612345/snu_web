...
window.openAdminDashboardOverview = async () => {
    ...
    content.innerHTML = `<div class="flex items-center justify-center p-20"><i class="fas fa-spinner fa-spin text-4xl text-blue-500"></i></div>`;
    ...
    try {
        ...
        const logEntries = logsSnap.docs.map(d => {
            ...
            return `<div class="flex items-start gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all group">
                <span class="text-xl flex-shrink-0">${icon}</span>
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-gray-800 dark:text-gray-200 truncate">${l.details || l.action}</p>
                    <p class="text-xs text-gray-400 mt-0.5">${l.adminEmail || '—'} · ${timeStr}</p>
                </div>
            </div>`;
        }).join('');
        ...
        content.innerHTML = `
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="bg-gradient-to-br from-emerald-500 to-green-700 text-white p-5 rounded-2xl shadow-lg relative overflow-hidden">
                        <div class="absolute -top-4 -left-4 w-20 h-20 bg-white/10 rounded-full"></div>
                    ...
                    <div class="bg-gradient-to-br from-indigo-500 to-violet-700 text-white p-5 rounded-2xl shadow-lg relative overflow-hidden">
                        <div class="absolute -top-4 -left-4 w-20 h-20 bg-white/10 rounded-full"></div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border dark:border-gray-700">
                        <h3 class="font-bold text-lg mb-4 text-gray-800 dark:text-white"><i class="fas fa-chart-bar text-blue-500 ml-2"></i>نشاط آخر 7 أيام</h3>
                        <div class="flex items-end gap-2 h-40">
                            ${days.map(d => {
            const pct = Math.max((d.count / maxActivity) * 100, 4);
            const isToday = d.date === now.toISOString().split('T')[0];
            return `<div class="flex-1 flex flex-col items-center gap-1">
                                    <span class="text-xs font-bold text-gray-600 dark:text-gray-300">${d.count}</span>
                                    <div class="w-full rounded-t-lg transition-all duration-500 ${isToday ? 'bg-gradient-to-t from-blue-600 to-indigo-500' : 'bg-gradient-to-t from-blue-400 to-indigo-300 dark:from-blue-600 dark:to-indigo-500'}" style="height: ${pct}%"></div>
                                    <span class="text-[10px] text-gray-400 ${isToday ? 'font-bold text-blue-600 dark:text-blue-400' : ''}">${d.label}</span>
                                </div>`;
        }).join('')}
                        </div>
                    </div>
                    ...
                    <!-- Admin Activity Log -->
                    <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border dark:border-gray-700">
                        <h3 class="font-bold text-lg mb-4 text-gray-800 dark:text-white"><i class="fas fa-clipboard-list text-amber-500 ml-2"></i>آخر نشاط الأدمن</h3>
                        <div class="space-y-1 max-h-48 overflow-y-auto custom-scrollbar">
                            ${logEntries || '<p class="text-gray-400 text-center py-6">لا يوجد سجل بعد</p>'}
                        </div>
                    </div>
                </div>

                <!-- Quick Actions -->
                <div class="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow border dark:border-gray-700">
                    <h3 class="font-bold mb-3 text-gray-700 dark:text-gray-300"><i class="fas fa-bolt text-yellow-500 ml-1"></i>إجراءات سريعة</h3>
                    <div class="flex flex-wrap gap-2">
                        <button onclick="window.openUsersLogView?.()" class="px-4 py-2 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 rounded-xl text-sm font-bold hover:bg-blue-200 transition"><i class="fas fa-users ml-1"></i>الطلاب</button>
                        <button onclick="window.openStatisticsDashboard?.()" class="px-4 py-2 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 rounded-xl text-sm font-bold hover:bg-indigo-200 transition"><i class="fas fa-chart-pie ml-1"></i>الإحصائيات</button>
                        ...
                        <button onclick="window.openAdminActivityLog?.()" class="px-4 py-2 bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300 rounded-xl text-sm font-bold hover:bg-gray-200 transition"><i class="fas fa-clipboard-list ml-1"></i>السجل الكامل</button>
                    </div>
                </div>
            </div>
        `;
...
export const openAssignmentsAdminView = async () => {
    ...
    content.innerHTML = `
        <div class="mb-10 p-8 bg-white dark:bg-gray-800 rounded-3xl shadow-xl border-t-8 border-blue-600 animate-fade-in">
            <h3 class="font-black text-2xl mb-6 dark:text-white flex items-center gap-2">
                <i class="fas fa-plus-circle text-blue-600"></i> نشر واجب جديد
            </h3>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label class="block text-xs font-bold mb-2 dark:text-gray-300 uppercase tracking-wide">عنوان التكليف:</label>
                    <input type="text" id="assign-title" placeholder="مثال: حل مسائل الشيت الأول" class="w-full p-4 border rounded-2xl dark:bg-gray-700 dark:text-white outline-none focus:ring-2 focus:ring-blue-500 font-bold">
                </div>
                <div>
                    <label class="block text-xs font-bold mb-2 dark:text-gray-300 uppercase tracking-wide">آخر موعد للتسليم:</label>
                    <input type="date" id="assign-date" class="w-full p-4 border rounded-2xl dark:bg-gray-700 dark:text-white outline-none focus:ring-2 focus:ring-blue-500 font-sans">
                </div>
            </div>

            <div class="mb-6">
                <label class="block text-xs font-bold mb-2 dark:text-gray-300 uppercase tracking-wide">المادة الدراسية (لربط الدرجات):</label>
                <select id="assign-section" class="w-full p-4 border rounded-2xl dark:bg-gray-700 dark:text-white outline-none focus:ring-2 focus:ring-blue-500 font-bold">
                    ${sectionsOptions}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label class="block text-xs font-bold mb-2 dark:text-gray-300 uppercase tracking-wide">الكلية المستهدفة:</label>
                    <select id="assign-col" class="w-full p-4 border rounded-2xl dark:bg-gray-700 dark:text-white outline-none font-bold">
                        <option value="">-- اختر الكلية --</option>
                        ${colOptions}
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-bold mb-2 dark:text-gray-300 uppercase tracking-wide">القسم / التخصص:</label>
                    <select id="assign-dept" class="w-full p-4 border rounded-2xl dark:bg-gray-700 dark:text-white outline-none font-bold">
            ...
            <div class="mb-8">
                <label class="block text-xs font-bold mb-2 dark:text-gray-300 uppercase tracking-wide">تعليمات الواجب:</label>
                <textarea id="assign-desc" placeholder="اكتب وصفاً دقيقاً للمطلوب من الطالب..." class="w-full p-4 border rounded-2xl dark:bg-gray-700 dark:text-white outline-none focus:ring-2 focus:ring-blue-500 h-32"></textarea>
            </div>
            
            <button id="create-assign-btn" class="w-full bg-blue-600 text-white py-4 rounded-2xl font-black shadow-lg hover:bg-blue-700 transition transform hover:-translate-y-1 flex items-center justify-center gap-2 text-lg">
        ...
        <div id="submissions-viewer" class="hidden fixed inset-0 bg-black/80 z-[250] flex items-center justify-center p-4 backdrop-blur-md">
            <div class="bg-white dark:bg-gray-800 w-full max-w-6xl h-[90vh] rounded-3xl flex flex-col relative shadow-2xl overflow-hidden border border-gray-700">
                <div class="p-6 border-b dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">قم بمراجعة الملفات ورصد الدرجات</p>
                    </div>
                ...
                <div id="submissions-list-content" class="flex-1 overflow-y-auto p-8 custom-scrollbar bg-gray-100 dark:bg-gray-800/50"></div>
            </div>
        </div>
    `;
...
const loadAdminAssignments = async () => {
    ...
    list.innerHTML = '<div class="text-center p-10"><i class="fas fa-spinner fa-spin text-3xl text-blue-600"></i></div>';

    const snap = await getDocs(query(collection(db, "assignments"), orderBy("createdAt", "desc"), limit(100)));
    list.innerHTML = '';

    if (snap.empty) {
        list.innerHTML = '<div class="p-12 bg-white dark:bg-gray-800 rounded-3xl text-center text-gray-400 font-bold border-2 border-dashed dark:border-gray-700">لا توجد واجبات نشطة حالياً.</div>';
        return;
    }
    ...
    snap.forEach(d => {
        ...
        div.className = "flex flex-col md:flex-row justify-between items-center p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-md border-r-8 border-blue-500 hover:shadow-xl transition group";
        div.innerHTML = `
            <div class="mb-4 md:mb-0 w-full">
                ...
                <div class="flex flex-wrap gap-2 text-xs font-bold uppercase tracking-wider">
                    <span class="text-blue-600 bg-blue-50 dark:bg-blue-900/30 px-3 py-1 rounded-lg">${data.sectionTitle}</span>
                    <span class="text-gray-500 bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-lg"><i class="far fa-clock"></i> ${data.deadline}</span>
                    <span class="text-blue-500 bg-blue-50 dark:bg-blue-900/30 px-3 py-1 rounded-lg">${data.collegeId} / ${data.departmentId}</span>
                </div>
            </div>
            <div class="flex gap-3 w-full md:w-auto">
                <button onclick="window.loadSubmissions('${d.id}')" class="flex-grow bg-blue-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-700 shadow-lg shadow-blue-500/30 transition flex items-center justify-center gap-2">
...
window.loadSubmissions = async (assignId) => {
    ...
    content.innerHTML = '<div class="text-center p-20"><i class="fas fa-spinner fa-spin text-4xl text-blue-600"></i><p class="mt-4 text-gray-500 font-bold">جاري تحميل إجابات الطلاب...</p></div>';
    ...
    if (snap.empty) {
        content.innerHTML = `
            <div class="flex flex-col items-center justify-center p-20 opacity-50">
                <i class="fas fa-folder-open text-8xl mb-6 text-gray-300"></i>
                <p class="text-2xl font-bold text-gray-500">لم يقم أي طالب برفع الحل حتى الآن.</p>
            </div>`;
        return;
    }
    ...
    snap.forEach(subDoc => {
        ...
        html += `
            <div class="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col lg:flex-row gap-6 transition hover:shadow-lg hover:border-blue-500">
                <div class="flex-1 flex gap-4">
                    <div class="w-16 h-16 bg-blue-100 dark:bg-blue-900/50 rounded-2xl flex items-center justify-center text-blue-600 text-2xl font-black shadow-inner">
                        ${sub.studentName ? sub.studentName.charAt(0) : '?'}
                    </div>
                    <div>
                        <h4 class="font-black text-xl dark:text-white mb-1">${sub.studentName}</h4>
                        <p class="text-xs text-gray-400 font-bold mb-4">تم التسليم: ${sub.submittedAt?.toDate().toLocaleString('ar-EG')}</p>
                        <a href="${sub.fileUrl}" target="_blank" class="inline-flex items-center gap-2 bg-blue-50 text-blue-700 px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-blue-600 hover:text-white transition border border-blue-100">
                            <i class="fas fa-file-download"></i> معاينة الملف المرفق
                        </a>
                    </div>
                </div>
                
                <div class="lg:w-80 bg-gray-50 dark:bg-gray-900 p-5 rounded-2xl border dark:border-gray-700 flex flex-col gap-3">
                    <div class="flex items-center gap-2">
                        <div class="flex-1">
                            <label class="block text-[10px] font-black text-gray-400 mb-1 uppercase">الدرجة (100)</label>
                            <input type="number" id="grade-${sub.userId}" value="${sub.grade || ''}" class="w-full p-3 bg-white dark:bg-gray-800 border rounded-xl text-center font-black text-xl outline-none focus:ring-2 focus:ring-green-500 text-green-600">
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-gray-400 mb-1 uppercase">ملاحظات (Feedback)</label>
                        <textarea id="feedback-${sub.userId}" class="w-full p-3 bg-white dark:bg-gray-800 border rounded-xl text-sm outline-none dark:text-white" rows="2" placeholder="اكتب ملاحظة للطالب...">${sub.feedback || ''}</textarea>
                    </div>
                    <button onclick="window.saveGrade('${assignId}', '${sub.userId}')" class="w-full ${isGraded ? 'bg-green-600' : 'bg-gray-800'} text-white py-3 rounded-xl font-bold shadow-lg hover:opacity-90 transition flex justify-center items-center gap-2">
...
export const openLiveSessionControl = async () => {
    ...
    document.getElementById('admin-view-content').innerHTML = `
        <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 p-10 rounded-[2.5rem] shadow-2xl text-center border-t-8 ${data.isActive ? 'border-red-500' : 'border-gray-300'} animate-fade-in">
            <div class="w-24 h-24 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center mx-auto mb-8 relative">
                <i class="fas fa-video text-5xl ${data.isActive ? 'text-red-500 animate-pulse' : 'text-gray-400'}"></i>
            ...
            <h3 class="text-3xl font-black mb-2 dark:text-white">
                حالة البث: ${data.isActive ? '<span class="text-red-600">مباشر الآن 📡</span>' : '<span class="text-gray-400">متوقف 💤</span>'}
            </h3>
            <p class="text-gray-500 mb-8 font-medium">عند تفعيل البث، سيظهر زر "مباشر الآن" في الشريط العلوي لجميع الطلاب.</p>
            
            <div class="bg-gray-50 dark:bg-gray-900 p-6 rounded-2xl mb-8 text-right">
                <label class="block text-xs font-black text-gray-400 mb-2 uppercase">رابط الاجتماع (Zoom / Google Meet / YouTube):</label>
                <div class="relative">
                    <i class="fas fa-link absolute top-4 left-4 text-gray-400"></i>
                    <input type="text" id="live-link" value="${data.link || ''}" placeholder="https://zoom.us/j/..." class="w-full p-4 pl-10 border rounded-xl bg-white dark:bg-gray-800 dark:text-white font-mono text-sm outline-none focus:ring-2 focus:ring-blue-500 dir-ltr text-left">
                </div>
            </div>
            
            <div class="flex gap-4">
                ${!data.isActive ? `
                    <button id="start-live" class="flex-1 bg-green-600 text-white py-4 rounded-2xl font-black hover:bg-green-700 text-lg shadow-lg shadow-green-500/30 transition transform hover:-translate-y-1 flex items-center justify-center gap-2">
...
export const openUsersLogView = async () => {
    ...
    const coreButtons = [];
    coreButtons.push(`<button onclick="window.openAdminDashboardOverview()" class="admin-btn group"><div class="admin-btn-icon bg-blue-500/20 text-blue-400"><i class="fas fa-tachometer-alt"></i></div><span>لوحة المعلومات</span></button>`);
    if (hasSupportAccess) coreButtons.push(`<button onclick="window.openSupportAdmin()" class="admin-btn group"><div class="admin-btn-icon bg-blue-500/20 text-blue-400"><i class="fas fa-headset"></i></div><span>${window.t?.('admin-support') || 'الدعم الفني'}</span></button>`);
    if (perms.idReview) coreButtons.push(`<button onclick="window.openIdReviewDashboard()" class="admin-btn group"><div class="admin-btn-icon bg-blue-500/20 text-blue-400"><i class="fas fa-id-card"></i></div><span>${window.t?.('admin-id-review') || 'مراجعة الهويات'}</span></button>`);
    ...
    if (perms.broadcast) alertButtons.push(`<button onclick="window.openBroadcastNotificationPanel()" class="admin-btn group"><div class="admin-btn-icon bg-indigo-500/20 text-indigo-400"><i class="fas fa-paper-plane"></i></div><span>إرسال إشعارات</span></button>`);
    ...
    const dataButtons = [];
    if (perms.grades) dataButtons.push(`<button onclick="window.viewUserScoresView()" class="admin-btn group"><div class="admin-btn-icon bg-blue-500/20 text-blue-400"><i class="fas fa-chart-line"></i></div><span>${window.t?.('grades-log') || 'سجل الدرجات'}</span></button>`);
    if (perms.statistics) dataButtons.push(`<button onclick="window.openStatisticsDashboard()" class="admin-btn group"><div class="admin-btn-icon bg-indigo-500/20 text-indigo-400"><i class="fas fa-chart-pie"></i></div><span>${window.t?.('statistics') || 'الإحصائيات'}</span></button>`);
    ...
    if (isGlobalOwner) dataButtons.push(`<button onclick="window.openAdvancedExport()" class="admin-btn group"><div class="admin-btn-icon bg-emerald-500/20 text-emerald-400"><i class="fas fa-file-export"></i></div><span>${window.t?.('advanced-export') || 'تصدير متقدم'}</span></button>`);
    ...
    if (perms.devices) securityButtons.push(`<button onclick="window.openDeviceSessionsView()" class="admin-btn group"><div class="admin-btn-icon bg-indigo-500/20 text-indigo-400"><i class="fas fa-mobile-alt"></i></div><span>${window.t?.('connected-devices') || 'الأجهزة المتصلة'}</span></button>`);
    if (perms.loginLogs) securityButtons.push(`<button onclick="window.openLoginLogsView()" class="admin-btn group"><div class="admin-btn-icon bg-gray-400/20 text-gray-400"><i class="fas fa-history"></i></div><span>${window.t?.('login-logs') || 'سجل الدخول'}</span></button>`);
    ...
    if (isGlobalOwner) securityButtons.push(`<button onclick="window.openAdminActivityLog()" class="admin-btn group"><div class="admin-btn-icon bg-gray-500/20 text-gray-400"><i class="fas fa-clipboard-list"></i></div><span>سجل نشاط الأدمن</span></button>`);
    if (isGlobalOwner) securityButtons.push(`<button onclick="window.openWatermarkExemptions()" class="admin-btn group"><div class="admin-btn-icon bg-indigo-500/20 text-indigo-400"><i class="fas fa-eye-slash"></i></div><span>إعفاء من العلامة المائية</span></button>`);
    ...
    const adminGridHTML = adminSections.map(s => `
        <div class="mb-4">
            <h4 class="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2 flex items-center gap-2 px-1">
                <i class="fas ${s.icon} text-blue-500"></i> ${s.title}
            </h4>
    ...
    content.innerHTML = `
        ...
        <div class="mb-6 flex gap-2 overflow-x-auto pb-2">
            <button onclick="window.openGradesUploadView()" class="bg-emerald-100 text-emerald-700 px-4 py-2 rounded-xl font-bold text-xs hover:bg-emerald-200 whitespace-nowrap"><i class="fas fa-file-excel"></i> رفع الدرجات</button>
            <button onclick="window.giveGlobalBonus()" class="bg-green-100 text-green-700 px-4 py-2 rounded-xl font-bold text-xs hover:bg-green-200 whitespace-nowrap"><i class="fas fa-gift"></i> ${window.t?.('bonus-all') || 'بونص للجميع'}</button>
            <button onclick="window.copyAllEmails()" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-xl font-bold text-xs hover:bg-gray-200 whitespace-nowrap"><i class="fas fa-copy"></i> ${window.t?.('copy-emails') || 'نسخ الإيميلات'}</button>
        ...
        <!-- فلاتر الطلاب -->
        <div class="mb-6 bg-white dark:bg-gray-800 p-4 rounded-2xl shadow border dark:border-gray-700">
            <div class="flex flex-wrap gap-3 items-center">
                <div class="flex-1 min-w-[200px]">
                    <div class="relative">
                        <i class="fas fa-search absolute top-1/2 -translate-y-1/2 right-3 text-gray-400"></i>
                        <input type="text" id="search-student" placeholder="بحث بالاسم، الإيميل، الرقم الجامعي، الرقم القومي..." class="w-full p-3 pr-10 border rounded-xl dark:bg-gray-700 dark:text-white font-bold text-sm focus:ring-2 focus:ring-blue-500 outline-none" oninput="window.filterStudentsBySearch()">
                    </div>
                </div>
                <div class="flex-1 min-w-[150px]">
                    <select id="filter-college" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-bold text-sm">
                        <option value="">🏫 كل الكليات</option>
                        ${UNIVERSITY_STRUCTURE.map(c => `<option value="${c.id}">${c.name}</option>`).join('')}
                    </select>
                </div>
                <div class="flex-1 min-w-[150px]">
                    <select id="filter-dept" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-bold text-sm" disabled>
                        <option value="">📚 كل الأقسام</option>
                    </select>
                </div>
                <div class="flex gap-2">
                    <button onclick="window.refreshUsersList()" class="bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-300 px-4 py-3 rounded-xl font-bold text-sm hover:bg-blue-200 transition" title="تحديث">
                        <i class="fas fa-sync-alt"></i>
                    </button>
                    <button onclick="window.copyFilteredEmails()" class="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-4 py-3 rounded-xl font-bold text-sm hover:bg-gray-200 transition" title="نسخ إيميلات">
                        <i class="fas fa-copy"></i>
                    </button>
                    <button onclick="window.exportUsersToExcel()" class="bg-green-600 text-white px-4 py-3 rounded-xl font-bold text-sm hover:bg-green-700 transition" title="تصدير Excel">
                        <i class="fas fa-file-excel"></i>
                    </button>
                    <button onclick="window.sendTargetedNotification()" class="bg-blue-600 text-white px-4 py-3 rounded-xl font-bold text-sm hover:bg-blue-700 transition" title="إرسال تنبيه">
            ...
            <div class="mt-3 flex items-center gap-4 text-xs text-gray-500">
                ...
                <label class="flex items-center gap-2 cursor-pointer hover:text-blue-600">
                    <input type="checkbox" id="filter-online" class="w-4 h-4 accent-green-500">
                    <span class="font-bold">🟢 Online أولاً</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer hover:text-blue-600">
                    <input type="checkbox" id="filter-verified" class="w-4 h-4 accent-blue-500">
                    <span class="font-bold">✅ موثقين فقط</span>
                </label>
            </div>
        </div>

        <div id="users-table-container" class="overflow-hidden bg-white dark:bg-gray-800 rounded-3xl shadow-lg border dark:border-gray-700">
            <div class="p-20 text-center text-gray-400"><i class="fas fa-circle-notch fa-spin text-4xl mb-4"></i><p>جاري تحميل قاعدة بيانات الطلاب...</p></div>
        </div>

        <div id="bonus-modal" class="hidden fixed inset-0 bg-black/70 z-[300] flex items-center justify-center p-4 backdrop-blur-sm">
            <div class="bg-white dark:bg-gray-800 rounded-[2rem] p-8 w-full max-w-md relative shadow-2xl border-t-8 border-green-500 animate-slide-up">
                <button onclick="document.getElementById('bonus-modal').classList.add('hidden')" class="absolute top-6 right-6 text-gray-400 hover:text-red-500 text-xl"><i class="fas fa-times"></i></button>
                <h3 class="text-2xl font-black mb-6 dark:text-white flex items-center gap-2"><i class="fas fa-star text-green-500"></i> مكافأة طالب</h3>
                <input type="hidden" id="bonus-user-id">
                <input type="hidden" id="bonus-user-name">
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-bold mb-1 dark:text-gray-300">اختر المادة:</label>
                        <select id="bonus-subject-select" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white outline-none font-bold"><option value="">جاري التحميل...</option></select>
                    </div>
                    <div>
                        <label class="block text-xs font-bold mb-1 dark:text-gray-300">عدد الدرجات:</label>
                        <input type="number" id="bonus-amount" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-bold text-center text-xl outline-none focus:ring-2 focus:ring-green-500" placeholder="5">
                    </div>
                    <div>
                        <label class="block text-xs font-bold mb-1 dark:text-gray-300">السبب (يظهر للطالب):</label>
                        <input type="text" id="bonus-reason" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white outline-none" placeholder="تفاعل ممتاز، إجابة صحيحة...">
                    </div>
                </div>
                <button id="confirm-bonus-btn" class="w-full mt-8 bg-green-600 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-green-700 transition">تأكيد وإضافة</button>
            </div>
        </div>

        <div id="badges-modal" class="hidden fixed inset-0 bg-black/70 z-[300] flex items-center justify-center p-4 backdrop-blur-sm">
            <div class="bg-white dark:bg-gray-800 rounded-[2rem] p-8 w-full max-w-md relative shadow-2xl border-t-8 border-yellow-400 animate-slide-up">
                <button onclick="document.getElementById('badges-modal').classList.add('hidden')" class="absolute top-6 right-6 text-gray-400 hover:text-red-500 text-xl"><i class="fas fa-times"></i></button>
                ...
                <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-xl">
                    <p class="text-xs font-bold text-gray-400 mb-2">إضافة وسام جديد:</p>
                    <div class="flex gap-2">
                        <input type="text" id="new-badge-icon" placeholder="Emoji" class="w-16 p-3 border rounded-xl text-center text-2xl dark:bg-gray-700 dark:text-white">
                        <input type="text" id="new-badge-title" placeholder="اسم الوسام (مثال: عبقري)" class="flex-1 p-3 border rounded-xl text-sm dark:bg-gray-700 dark:text-white font-bold">
                    </div>
                    <button id="add-badge-confirm" class="w-full mt-3 bg-blue-600 text-white py-2 rounded-xl font-bold shadow hover:bg-blue-700 transition">إمنح الوسام</button>
                </div>
            </div>
        </div>
    `;
    ...
    try {
        ...
        if (usersSnap.empty) {
            document.getElementById('users-table-container').innerHTML = '<div class="p-20 text-center text-gray-400 font-bold">لا يوجد طلاب مسجلين.</div>';
            return;
        }
        ...
        usersSnap.forEach(userDoc => {
            ...
            const badgesDisplay = userData.badges ? userData.badges.map(b => `<span title="${b.title}" class="cursor-help text-lg hover:scale-125 inline-block transition">${b.icon}</span>`).join(' ') : '<span class="text-gray-300 text-xs">-</span>';

            rows += `
                <tr class="group border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition ${isPlatformBanned ? 'bg-red-50 dark:bg-red-900/10' : ''}">
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <div class="font-black dark:text-white text-sm flex items-center gap-2">
                                    <a href="#profile/${userData.uid}" class="hover:text-blue-600 transition">${userData.displayName}</a>
                                    ${isVerified ? '<i class="fas fa-check-circle text-blue-500 text-xs" title="موثق"></i>' : ''}
                                    ${isPlatformBanned ? '<span class="text-[9px] bg-red-600 text-white px-1.5 rounded">محظور</span>' : ''}
                                    <button onclick="window.editUserName('${userData.uid}', '${userData.displayName}')" class="text-xs text-gray-400 hover:text-blue-500 transition" title="تعديل الاسم"><i class="fas fa-pen"></i></button>
                                </div>
                                <div class="text-[10px] text-gray-500 font-mono">${isGlobalOwner ? userData.email : ''}</div>
                                ${isOnline ? '<span class="text-[10px] text-green-500 font-bold"><i class="fas fa-circle text-[6px] mr-1"></i>متصل</span>' : (userData.lastSeen ? `<span class="text-[10px] text-gray-400">${window._adminTimeAgo?.(userData.lastSeen) || ''}</span>` : '')}
                                <div class="text-[10px] text-blue-400 font-bold">${userData.collegeId || '?'} / ${userData.departmentId || '?'}</div>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex justify-end gap-2 flex-wrap">
                            ...
                            <button onclick="window.openBonusModal('${userData.uid}', '${userData.displayName}')" class="w-8 h-8 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition flex items-center justify-center" title="إضافة درجات"><i class="fas fa-plus"></i></button>
                            <button onclick="window.changeStudentCollege('${userData.uid}', '${userData.displayName}')" class="w-8 h-8 bg-blue-100 text-indigo-700 rounded-lg hover:bg-blue-200 transition flex items-center justify-center" title="نقل الطالب"><i class="fas fa-exchange-alt"></i></button>
                            ...
                            <button onclick="window.toggleChatBan('${userData.uid}', ${isChatBanned})" class="w-8 h-8 rounded-lg transition flex items-center justify-center ${isChatBanned ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/30' : 'bg-gray-100 text-gray-500 hover:bg-orange-100 hover:text-orange-600'}" title="${isChatBanned ? 'فك حظر الشات' : 'حظر الشات'}">
                                <i class="fas fa-comment-slash"></i>
                            </button>
                            
                            ${(isGlobalOwner || perms.bannedUsers) ? `
                                <button onclick="window.togglePlatformBan('${userData.uid}', ${isPlatformBanned})" class="w-8 h-8 rounded-lg transition flex items-center justify-center ${isPlatformBanned ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'bg-gray-100 text-gray-500 hover:bg-red-100 hover:text-red-600'}" title="${isPlatformBanned ? 'فك الحظر النهائي' : 'طرد نهائي'}">
                                    <i class="fas fa-ban"></i>
                                </button>
                            ` : ''}
                            ${(isGlobalOwner || perms.deleteUsers) ? `
                                <button onclick="window.deleteUserPermanently('${userData.uid}', '${userData.displayName?.replace(/'/g, "\\'") || 'هذا المستخدم'}', '${userData.email || ''}')" class="w-8 h-8 bg-gray-800 text-white rounded-lg hover:bg-black transition flex items-center justify-center shadow-lg" title="حذف نهائي ❌">
        ...
        if (tableContainer) {
            tableContainer.innerHTML = `
                <table class="w-full text-sm text-right">
                    <thead class="bg-gray-50 dark:bg-gray-700/50 text-gray-500 dark:text-gray-300 font-bold uppercase text-xs">
                        <tr>
                    ...
                    <tbody class="divide-y dark:divide-gray-700 bg-white dark:bg-gray-800">${rows}</tbody>
                </table>`;
        }
...
window.filterStudents = () => {
    ...
    */
    ...
    if (filtered.length === 0) {
        tableContainer.innerHTML = `
            <div class="p-16 text-center">
                ...
                <h3 class="text-lg font-bold text-gray-400">لا توجد نتائج</h3>
                <p class="text-sm text-gray-400 mt-1">جرب البحث باسم آخر أو رقم جامعي أو رقم قومي</p>
            </div>`;
        return;
    }
    ...
    filtered.forEach(userData => {
        ...
        // تظليل نتيجة البحث
        const searchHighlight = searchVal ? ` <span class="text-[9px] text-blue-400">` +
                ...
                <td class="px-6 py-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div class="text-[10px] text-gray-500 font-mono">${isGlobalOwner ? userData.email : ''}</div>
                            ${isOnline ? '<span class="text-[10px] text-green-500 font-bold"><i class="fas fa-circle text-[6px] mr-1"></i>متصل</span>' : (userData.lastSeen ? `<span class="text-[10px] text-gray-400">${window._adminTimeAgo?.(userData.lastSeen) || ''}</span>` : '')}
                            <div class="text-[10px] text-blue-400 font-bold">${userData.collegeId || '?'} / ${userData.departmentId || '?'}${searchHighlight}</div>
                        </div>
                    </div>
                </td>
                <td class="px-4 py-4 text-center">${badgesDisplay}</td>
...
export const openIdReviewDashboard = async () => {
    ...
    content.innerHTML = '<div class="text-center p-20"><i class="fas fa-spinner fa-spin text-4xl text-blue-600"></i></div>';
    ...
    try {
        ...
        if (snap.empty) {
            content.innerHTML = `
                <div class="text-center py-20">
                    ...
                    <h3 class="text-xl font-bold text-gray-600 dark:text-gray-400 mb-2">لا توجد طلبات</h3>
                    <p class="text-gray-400">لم يقم أي طالب برفع هويته بعد</p>
                </div>`;
            return;
        }
        ...
        <div class="flex flex-wrap gap-3 mb-4 items-center">
            <button onclick="window.refreshIdReview()" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold transition flex items-center gap-2 shadow-lg">
                <i class="fas fa-sync-alt"></i> تحديث
            </button>
            ${pendingUsers.length > 0 ? `
            <button onclick="window.toggleSelectAllIds()" id="select-all-btn" class="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-5 py-3 rounded-xl font-bold transition flex items-center gap-2 hover:bg-gray-300">
                <i class="far fa-check-square"></i> تحديد الكل
            </button>
            <button onclick="window.batchVerifySelected()" id="batch-approve-btn" class="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-5 py-3 rounded-xl font-bold transition flex items-center gap-2 shadow-lg hover:shadow-green-500/30 disabled:opacity-50" disabled>
                <i class="fas fa-check-double"></i> <span id="batch-approve-text">قبول المحددين (0)</span>
            </button>
            <button onclick="window.batchVerifyAll()" class="bg-gradient-to-r from-blue-600 to-blue-600 text-white px-5 py-3 rounded-xl font-bold transition flex items-center gap-2 shadow-lg hover:shadow-blue-500/30">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <div class="bg-gradient-to-br from-green-500 to-emerald-600 text-white p-5 rounded-2xl shadow-lg">
            ...
            <div class="bg-gradient-to-br from-indigo-500 to-pink-600 text-white p-5 rounded-2xl shadow-lg">
        ...
        <!-- التبويبات -->
        <div class="flex gap-2 mb-6 bg-gray-100 dark:bg-gray-800 p-2 rounded-2xl">
            ...
            <button id="tab-verified" onclick="window.showIdTab('verified')" class="flex-1 py-3 px-4 rounded-xl font-bold flex items-center justify-center gap-2 bg-transparent text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 transition">
        ...
        <div id="pending-list" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            ${pendingUsers.length === 0 ? `
                <div class="col-span-full text-center py-16 bg-green-50 dark:bg-green-900/20 rounded-2xl">
                    <i class="fas fa-check-double text-5xl text-green-500 mb-4"></i>
                    <h3 class="text-lg font-bold text-green-600">لا توجد طلبات معلقة 🎉</h3>
                    <p class="text-sm text-gray-500">جميع الطلبات تمت مراجعتها</p>
                </div>
        ...
        <div id="verified-list" class="hidden">
            ...
            <div class="mb-4 flex gap-2">
                <button onclick="window.showAllVerified()" class="flex-1 bg-gradient-to-r from-green-500 to-emerald-600 text-white py-3 px-6 rounded-xl font-bold hover:shadow-lg transition flex items-center justify-center gap-2">
                    <i class="fas fa-th-large"></i> عرض الكل (${verifiedUsers.length})
                </button>
                <button onclick="window.showVerifiedByCollege()" class="flex-1 bg-gradient-to-r from-blue-500 to-indigo-600 text-white py-3 px-6 rounded-xl font-bold hover:shadow-lg transition flex items-center justify-center gap-2">
            ...
            <div id="verified-by-college" class="hidden space-y-4">
                ...
                return UNIVERSITY_STRUCTURE.map(college => {
                    ...
                    return `
                        <div class="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-lg border-r-4 border-green-500">
                            <div class="bg-gradient-to-r from-green-500 to-emerald-600 text-white p-4 cursor-pointer flex items-center justify-between hover:brightness-110 transition" onclick="document.getElementById('college-verified-${college.id}').classList.toggle('hidden')">
                            ...
                            <div id="college-verified-${college.id}" class="hidden p-4 grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-700/50">
                                ${users.map(u => renderIdCard(u, true)).join('')}
                            </div>
                        </div>`;
                }).join('') + (byCollege['unknown']?.length ? `
                        <div class="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-lg border-r-4 border-gray-400">
                            <div class="bg-gradient-to-r from-gray-500 to-gray-600 text-white p-4 cursor-pointer flex items-center justify-between hover:brightness-110 transition" onclick="document.getElementById('college-verified-unknown').classList.toggle('hidden')">
                                <div class="flex items-center gap-3">
                            ...
                            <div id="college-verified-unknown" class="hidden p-4 grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-700/50">
...
const renderIdCard = (u, isVerified) => {
    ...
    return `
    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl overflow-hidden border-2 ${isVerified ? 'border-green-500' : 'border-orange-400'} flex flex-col group hover:shadow-2xl transition-all duration-300 hover:-translate-y-1" id="id-card-${u.id}">
        <!-- صورة الكارنيه -->
        <div class="relative h-52 bg-gray-200 dark:bg-gray-700 cursor-pointer overflow-hidden" onclick="window.open('${u.idCardImage}', '_blank')">
            ...
            <!-- Badge -->
            <div class="absolute top-3 right-3 ${isVerified ? 'bg-green-500' : 'bg-orange-500'} text-white px-3 py-1.5 rounded-full text-xs font-bold shadow-lg flex items-center gap-1">
            ...
            <label class="absolute top-3 left-3 cursor-pointer" onclick="event.stopPropagation()">
                <input type="checkbox" class="id-select-checkbox w-6 h-6 accent-green-500 rounded-lg cursor-pointer shadow-lg" data-uid="${u.id}" onchange="window.updateBatchCount()">
            </label>
            ` : ''}
        </div>
        ...
        <div class="p-5 flex-1 flex flex-col">
            <div class="flex items-center gap-3 mb-4">
                <img src="${u.photoURL || 'https://ui-avatars.com/api/?name=' + u.displayName}" loading="lazy" class="w-12 h-12 rounded-full border-2 ${isVerified ? 'border-green-500' : 'border-orange-400'} shadow">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-[11px] text-gray-500 truncate font-mono">${(auth.currentUser?.email === SUPER_ADMIN_EMAIL) ? u.email : (u.collegeId || '')}</p>
                </div>
            </div>
            ...
            <div class="space-y-2 mb-4 text-xs">
                ${u.nationalId ? `
                <div class="flex items-center justify-between bg-blue-50 dark:bg-blue-900/30 p-2 rounded-lg">
                    <span class="text-blue-600 dark:text-blue-400 font-bold">🆔 الرقم القومي</span>
                    <span class="font-mono font-bold text-gray-800 dark:text-gray-200 tracking-wider">${u.nationalId}</span>
                </div>
                ` : ''}
                ${u.studentId ? `
                <div class="flex items-center justify-between bg-blue-50 dark:bg-indigo-900/30 p-2 rounded-lg">
                    <span class="text-indigo-600 dark:text-indigo-400 font-bold">🎓 رقم الطالب</span>
                    <span class="font-mono font-bold text-gray-800 dark:text-gray-200 tracking-wider">${u.studentId}</span>
                </div>
                ` : ''}
                ${u.fullName ? `
                <div class="flex items-center justify-between bg-green-50 dark:bg-green-900/30 p-2 rounded-lg">
                    <span class="text-green-600 dark:text-green-400 font-bold">👤 الاسم الرباعي</span>
                    <span class="font-bold text-gray-800 dark:text-gray-200 text-[11px]">${u.fullName}</span>
                </div>
                ` : ''}
                <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 p-2 rounded-lg">
                    <span class="text-gray-500">الكلية / القسم</span>
                    <span class="font-bold text-gray-700 dark:text-gray-300">${collegeInfo}</span>
                </div>
                <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 p-2 rounded-lg">
                    <span class="text-gray-500">تاريخ الرفع</span>
                    <span class="font-mono text-gray-700 dark:text-gray-300">${uploadTime}</span>
                </div>
            </div>
            ...
            <div class="flex gap-2 mt-auto">
                ${isVerified ? `
                    <button onclick="window.verifyUser('${u.id}', false)" class="flex-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 py-3 rounded-xl font-bold hover:bg-gray-200 dark:hover:bg-gray-600 transition text-sm">
                        <i class="fas fa-times"></i> إلغاء التوثيق
                    </button>
                ` : `
                    <button onclick="window.verifySingleUser('${u.id}')" class="flex-1 bg-gradient-to-r from-green-500 to-emerald-600 text-white py-3 rounded-xl font-bold hover:shadow-lg hover:shadow-green-500/30 transition text-sm flex items-center justify-center gap-2">
                ...
                `}
                <button onclick="window.location.hash='#profile/${u.id}'" class="bg-blue-100 dark:bg-blue-900/30 text-blue-600 px-4 rounded-xl font-bold hover:bg-blue-200 transition" title="عرض البروفايل">
...
window.showIdTab = (tab) => {
    ...
    if (tab === 'pending') {
        ...
        tabPending.classList.remove('bg-transparent', 'text-gray-600', 'dark:text-gray-400');
        tabVerified.classList.remove('bg-green-500', 'text-white');
        tabVerified.classList.add('bg-transparent', 'text-gray-600', 'dark:text-gray-400');
    } else {
        pendingList.classList.add('hidden');
        verifiedList.classList.remove('hidden');
        tabVerified.classList.add('bg-green-500', 'text-white');
        tabVerified.classList.remove('bg-transparent', 'text-gray-600', 'dark:text-gray-400');
        tabPending.classList.remove('bg-orange-500', 'text-white');
        tabPending.classList.add('bg-transparent', 'text-gray-600', 'dark:text-gray-400');
    }
};
...
export const viewUserScoresView = async () => {
    ...
    content.innerHTML = '<div class="p-20 text-center"><i class="fas fa-spinner fa-spin text-4xl text-blue-600"></i><p class="mt-4 font-bold text-gray-500">جاري تحليل البيانات...</p></div>';
    ...
    try {
        ...
        if (isPageAdmin) {
            let html = `
                <div class="mb-6 flex justify-between items-center bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm">
                    <button onclick="window.openUsersLogView()" class="text-blue-600 font-bold hover:underline flex items-center gap-2"><i class="fas fa-arrow-right"></i> عودة للطلاب</button>
                    <button onclick="window.exportToExcel()" class="bg-green-600 text-white px-6 py-3 rounded-xl shadow-lg font-black hover:bg-green-700 transition flex items-center gap-2 transform hover:scale-105">
                        <i class="fas fa-file-excel text-xl"></i> تحميل تقرير Excel
                    </button>
                </div>
                <div class="overflow-x-auto rounded-3xl shadow-xl border dark:border-gray-700 bg-white dark:bg-gray-800">
                    <table class="w-full text-sm text-right">
                        <thead class="bg-gray-100 dark:bg-gray-900 font-black text-gray-600 dark:text-gray-300">
                            <tr>
            ...
            myScores.forEach(s => {
                html += `
                    <tr class="hover:bg-blue-50 dark:hover:bg-blue-900/10 transition">
                        <td class="p-5 font-bold dark:text-white">${s.userName}</td>
                        <td class="p-5 text-xs font-bold text-blue-500 bg-blue-50 dark:bg-blue-900/30 rounded w-fit h-fit">${s.collegeId || '-'}</td>
                        <td class="p-5">
                            <div class="font-bold dark:text-gray-300">${s.quizTitle}</div>
                            <div class="text-[10px] text-gray-400">${s.sectionTitle || 'عام'}</div>
                        </td>
                        <td class="p-5 text-center font-black text-lg ${s.score >= 50 ? 'text-green-600' : 'text-red-500'} dir-ltr">${s.score}</td>
                        <td class="p-5 text-xs text-gray-400 font-mono">${s.date?.toDate ? s.date.toDate().toLocaleDateString('ar-EG') : '-'}</td>
                    </tr>`;
            });
        ...
        for (const [subject, data] of Object.entries(grouped)) {
            ...
            studentHtml += `
                <div class="bg-white dark:bg-gray-800 rounded-[2rem] shadow-xl border-r-8 border-blue-600 overflow-hidden group hover:-translate-y-1 transition duration-300">
                    <div class="p-6 flex justify-between items-center cursor-pointer bg-gray-50 dark:bg-gray-700/50" onclick="document.getElementById('det-${sid}').classList.toggle('hidden')">
                        <div class="flex items-center gap-4">
                            <div class="w-14 h-14 bg-blue-600 text-white rounded-2xl flex items-center justify-center font-bold text-2xl shadow-lg shadow-blue-500/30">
                                <i class="fas fa-book"></i>
                            </div>
                            <div>
                                <h3 class="font-black text-xl dark:text-white">${subject}</h3>
                                <p class="text-xs text-gray-500 font-bold">${data.items.length} نشاط مسجل</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <span class="block text-[10px] text-gray-400 font-bold uppercase">المجموع</span>
                            <span class="text-3xl font-black text-green-600 dir-ltr">${data.total}</span>
                        </div>
                    </div>
                    
                    <div id="det-${sid}" class="hidden bg-white dark:bg-gray-800 p-6 animate-slide-down border-t dark:border-gray-700">
                        <table class="w-full text-sm text-right">
                            <thead class="text-xs text-gray-400 border-b dark:border-gray-700 uppercase font-black">
                                <tr><th class="pb-3">النشاط</th><th class="pb-3 text-center">الدرجة</th><th class="pb-3 text-left">التاريخ</th></tr>
                            </thead>
                            <tbody class="divide-y dark:divide-gray-700">
                                ${data.items.map(it => `
                                    <tr class="group/row hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        <td class="py-3 dark:text-white font-bold text-xs">${it.quizTitle}</td>
                                        <td class="py-3 text-center font-black ${it.score >= 50 ? 'text-green-500' : 'text-red-500'} dir-ltr">${it.score}</td>
                                        <td class="py-3 text-left text-[10px] text-gray-400 font-mono">${it.date?.toDate().toLocaleDateString()}</td>
                                    </tr>
                                `).join('')}
                            </tbody>
                        </table>
...
export const openAdminManagementView = async () => {
    ...
    if (auth.currentUser.email !== SUPER_ADMIN_EMAIL) {
        document.getElementById('admin-settings-content').innerHTML = `
            <div class="flex flex-col items-center justify-center p-20 bg-white dark:bg-gray-800 rounded-3xl shadow-xl">
                <i class="fas fa-lock text-6xl text-red-500 mb-6"></i>
                <h2 class="text-2xl font-black text-gray-800 dark:text-white">منطقة محظورة</h2>
                <p class="text-gray-500 mt-2">إدارة المشرفين متاحة فقط لمالك المنصة.</p>
            </div>`;
        return;
    }
    ...
    document.getElementById('admin-settings-content').innerHTML = `
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
            <div class="p-8 bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-xl h-fit border-t-8 border-blue-600">
                <h3 id="admin-form-title" class="font-black text-2xl mb-6 dark:text-white flex items-center gap-2">
                    <i class="fas fa-user-plus text-blue-600"></i> إضافة مشرف جديد
                </h3>
                
                <div class="mb-6">
                    <label class="block text-xs font-bold text-gray-400 mb-2 uppercase">البريد الإلكتروني (Google Email):</label>
                    <input type="email" id="admin-email" placeholder="example@gmail.com" class="w-full p-4 border rounded-2xl dark:bg-gray-700 dark:text-white text-left dir-ltr font-bold outline-none focus:ring-2 focus:ring-blue-500 font-mono">
                </div>
                
                <!-- اختيار الرتبة -->
                <div class="mb-6">
                    <label class="block text-xs font-bold text-gray-400 mb-2 uppercase">قالب الصلاحيات (يمكنك التعديل من الخيارات أدناه):</label>
                    <select id="admin-role" class="w-full p-4 border rounded-2xl dark:bg-gray-700 dark:text-white font-bold outline-none focus:ring-2 focus:ring-blue-500" onchange="window.applyRoleTemplate()">
                ...
                <div class="bg-gray-50 dark:bg-gray-900 p-6 rounded-2xl mb-6 border dark:border-gray-700">
                    <p class="text-xs font-black text-blue-600 mb-4 uppercase tracking-wide flex items-center gap-2"><i class="fas fa-sitemap"></i> تحديد النطاق (لإدارة الطلاب):</p>
                    
                    <div class="flex gap-3 mb-4">
                        <select id="scope-college" class="w-1/2 p-3 border rounded-xl text-xs dark:bg-gray-700 dark:text-white outline-none font-bold"><option value="">-- كل الكليات --</option>${collegesOptions}</select>
                        <select id="scope-dept" class="w-1/2 p-3 border rounded-xl text-xs dark:bg-gray-700 dark:text-white outline-none font-bold" disabled><option value="">-- كل الأقسام --</option></select>
                    </div>
                    
                    <div id="scope-checkboxes" class="h-40 overflow-y-auto bg-white dark:bg-gray-800 p-3 rounded-xl border dark:border-gray-700 custom-scrollbar space-y-1">
                        <p class="text-center text-xs text-gray-400 py-10 opacity-50">اختر الكلية لعرض المواد المتاحة (للمحتوى)...</p>
                    </div>
                </div>
                
                <div class="mb-6">
                    <p class="text-xs font-bold text-gray-400 mb-2 uppercase">المواد المحددة (لصلاحية المحتوى):</p>
                ...
                <!-- صلاحيات عادية -->
                <p class="text-xs font-bold text-gray-500 mb-2 uppercase">🟢 الصلاحيات الأساسية:</p>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
                    <label class="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-xl cursor-pointer hover:bg-blue-100 transition border border-blue-100 dark:border-blue-800">
                        <input type="checkbox" id="p-cms" class="w-4 h-4 accent-blue-600 rounded">
                        <span class="text-xs font-bold dark:text-white"><i class="fas fa-edit text-blue-500 mr-1"></i>المحتوى</span>
                    </label>
                    <label class="flex items-center gap-2 p-2 bg-blue-50 dark:bg-indigo-900/20 rounded-xl cursor-pointer hover:bg-blue-100 transition border border-blue-100 dark:border-indigo-800">
                        <input type="checkbox" id="p-quiz" class="w-4 h-4 accent-indigo-600 rounded">
                        <span class="text-xs font-bold dark:text-white"><i class="fas fa-question-circle text-indigo-500 mr-1"></i>الكويزات</span>
                    </label>
                    <label class="flex items-center gap-2 p-2 bg-green-50 dark:bg-green-900/20 rounded-xl cursor-pointer hover:bg-green-100 transition border border-green-100 dark:border-green-800">
                        <input type="checkbox" id="p-users" class="w-4 h-4 accent-green-600 rounded">
                        <span class="text-xs font-bold dark:text-white"><i class="fas fa-users text-green-500 mr-1"></i>الطلاب</span>
                    </label>
                    <label class="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-xl cursor-pointer hover:bg-blue-100 transition border border-blue-100 dark:border-blue-800">
                        <input type="checkbox" id="p-support" class="w-4 h-4 accent-blue-600 rounded">
                        <span class="text-xs font-bold dark:text-white"><i class="fas fa-headset text-blue-500 mr-1"></i>الدعم</span>
                    </label>
                    <label class="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-xl cursor-pointer hover:bg-blue-100 transition border border-blue-100 dark:border-blue-800">
                        <input type="checkbox" id="p-idReview" class="w-4 h-4 accent-blue-600 rounded">
                        <span class="text-xs font-bold dark:text-white"><i class="fas fa-id-card text-blue-500 mr-1"></i>الهويات</span>
                    </label>
                    <label class="flex items-center gap-2 p-2 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl cursor-pointer hover:bg-indigo-100 transition border border-indigo-100 dark:border-indigo-800">
                        <input type="checkbox" id="p-statistics" class="w-4 h-4 accent-indigo-600 rounded">
                        <span class="text-xs font-bold dark:text-white"><i class="fas fa-chart-pie text-indigo-500 mr-1"></i>الإحصائيات</span>
                    </label>
                    ...
                    <label class="flex items-center gap-2 p-2 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl cursor-pointer hover:bg-emerald-100 transition border border-emerald-100 dark:border-emerald-800">
                        <input type="checkbox" id="p-announcements" class="w-4 h-4 accent-emerald-600 rounded">
                        <span class="text-xs font-bold dark:text-white"><i class="fas fa-scroll text-emerald-500 mr-1"></i>الإعلانات</span>
                    </label>
                    <label class="flex items-center gap-2 p-2 bg-slate-50 dark:bg-slate-900/20 rounded-xl cursor-pointer hover:bg-slate-100 transition border border-slate-100 dark:border-slate-800">
                        <input type="checkbox" id="p-grades" class="w-4 h-4 accent-slate-600 rounded">
                        <span class="text-xs font-bold dark:text-white"><i class="fas fa-graduation-cap text-slate-500 mr-1"></i>رفع الدرجات</span>
                    </label>
                </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-6 p-3 bg-red-50 dark:bg-red-900/10 rounded-xl border-2 border-red-200 dark:border-red-800">
                    <label class="flex items-center gap-2 p-2 bg-white dark:bg-gray-800 rounded-xl cursor-pointer hover:bg-red-100 transition border border-red-200 dark:border-red-700">
                ...
                <div class="flex gap-3">
                    <button id="save-admin-btn" class="flex-1 bg-blue-600 text-white py-4 rounded-2xl hover:bg-blue-700 font-black shadow-lg shadow-blue-500/30 transition transform hover:-translate-y-1">
                        حفظ الصلاحيات
                    </button>
                    <button id="cancel-edit-admin" class="hidden bg-gray-200 text-gray-600 px-6 rounded-2xl hover:bg-gray-300 font-bold transition">إلغاء</button>
                </div>
            </div>
            
            <div class="p-8 bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-xl border-t-8 border-gray-400 h-fit">
                <h3 class="font-black text-2xl mb-6 dark:text-white flex items-center gap-2"><i class="fas fa-users-cog text-gray-500"></i> فريق الإدارة</h3>
                <div id="admins-list" class="space-y-3 max-h-[600px] overflow-y-auto custom-scrollbar pr-2"></div>
            </div>
            
            <!-- قسم الإشعارات -->
            <div class="lg:col-span-2 p-8 bg-gradient-to-br from-blue-50 to-blue-50 dark:from-blue-900/20 dark:to-blue-900/20 rounded-[2.5rem] shadow-xl border-2 border-blue-200 dark:border-blue-800">
                <h3 class="font-black text-2xl mb-6 text-blue-600 dark:text-blue-400 flex items-center gap-3">
                    <div class="w-12 h-12 bg-blue-600 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/30">
                        <i class="fas fa-bell"></i>
                    </div>
                    إعدادات الإشعارات
                </h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 border dark:border-gray-700 shadow-sm">
                        <h4 class="font-bold text-lg mb-4 dark:text-white flex items-center gap-2">
                            <i class="fas fa-mobile-alt text-green-500"></i> إشعارات الهاتف
                        </h4>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
                            فعّل الإشعارات لتصلك تنبيهات فورية على هاتفك.
                        </p>
                        <button onclick="window.showNotificationPrompt()" class="w-full bg-green-600 hover:bg-green-700 text-white py-4 px-6 rounded-2xl font-black text-lg transition transform hover:scale-105 hover:shadow-lg hover:shadow-green-500/30 flex items-center justify-center gap-3">
                            <i class="fas fa-bell"></i>
                            تفعيل الإشعارات
                        </button>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 border dark:border-gray-700 shadow-sm">
                        <h4 class="font-bold text-lg mb-4 dark:text-white flex items-center gap-2">
                            <i class="fas fa-info-circle text-blue-500"></i> معلومات
                        </h4>
                        <ul class="text-sm text-gray-500 dark:text-gray-400 space-y-2">
                            <li><i class="fas fa-check text-green-500 ml-2"></i>إشعارات المنشورات الجديدة</li>
                            <li><i class="fas fa-check text-green-500 ml-2"></i>إشعارات رسائل الدعم</li>
                            <li><i class="fas fa-check text-green-500 ml-2"></i>تنبيهات الكارنيهات المعلقة</li>
                            <li><i class="fas fa-check text-green-500 ml-2"></i>التنبيهات العاجلة</li>
                        </ul>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 p-8 bg-gradient-to-br from-red-50 to-orange-50 dark:from-red-900/20 dark:to-orange-900/20 rounded-[2.5rem] shadow-xl border-2 border-red-200 dark:border-red-800">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 border dark:border-gray-700 shadow-sm">
                        <h4 class="font-bold text-lg mb-4 dark:text-white flex items-center gap-2">
                            <i class="fas fa-comment-dots text-blue-500"></i> إعدادات الصيانة
                        </h4>
                        
                        <input type="text" id="maintenance-title" placeholder="عنوان الصفحة (الافتراضي: المنصة في وضع الصيانة)" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white text-sm outline-none focus:ring-2 focus:ring-red-500 mb-3">
                        
                        <select id="maintenance-layout" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white text-sm outline-none focus:ring-2 focus:ring-red-500 mb-3">
                        ...
                        <textarea id="maintenance-message" rows="2" placeholder="نعمل على تحسينات للمنصة..." class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white text-sm outline-none focus:ring-2 focus:ring-red-500 mb-3"></textarea>
                        <input type="text" id="maintenance-return-time" placeholder="الوقت المتوقع (مثال: اليوم الساعة 10 مساءً)" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white text-sm outline-none focus:ring-2 focus:ring-red-500 mb-3">
                        
                        <!-- رقم الواتساب -->
                        <div class="flex gap-3 mb-3">
                            <label class="flex items-center gap-2 cursor-pointer bg-gray-50 dark:bg-gray-700 px-4 py-3 rounded-xl border dark:border-gray-600 w-1/2">
                                <input type="checkbox" id="maintenance-show-whatsapp" class="w-5 h-5 accent-green-500" checked>
                                <span class="text-xs font-bold dark:text-white">عرض زر الواتساب</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer bg-gray-50 dark:bg-gray-700 px-4 py-3 rounded-xl border dark:border-gray-600 w-1/2">
                                <input type="checkbox" id="maintenance-show-phone" class="w-5 h-5 accent-blue-500" checked>
                                <span class="text-xs font-bold dark:text-white">الرقم نصياً</span>
                            </label>
                        </div>
                        <input type="text" id="maintenance-whatsapp" placeholder="رقم الواتساب (مثال: 01040224684)" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white text-sm outline-none focus:ring-2 focus:ring-green-500 mb-3">
                        
                        <button onclick="window.saveMaintenanceSettings()" class="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold transition">
                ...
                <div class="mt-6 bg-white dark:bg-gray-800 rounded-2xl p-6 border dark:border-gray-700 shadow-sm">
                    <h4 class="font-bold text-lg mb-4 dark:text-white flex items-center gap-2">
                        <i class="fas fa-user-shield text-green-500"></i> إيميلات يُسمح لها بالدخول أثناء الصيانة
                    </h4>
                    ...
                    <div class="flex gap-2 mb-4">
                        <input type="email" id="whitelist-email-input" placeholder="example@gmail.com" class="flex-1 p-3 border rounded-xl dark:bg-gray-700 dark:text-white text-sm outline-none focus:ring-2 focus:ring-green-500 font-mono text-left">
                        <button onclick="window.addWhitelistEmail()" class="bg-green-600 hover:bg-green-700 text-white px-6 rounded-xl font-bold transition flex items-center gap-2">
                            <i class="fas fa-plus"></i> أضف
                        </button>
                    </div>
                    
                    <div id="whitelist-emails-list" class="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                        <p class="text-center text-xs text-gray-400 py-4">جاري التحميل...</p>
                    </div>
                </div>
...
const renderScopeList = () => {
    ...
    if (!colId) {
        checkboxesDiv.innerHTML = '<p class="text-center text-xs text-gray-400 py-4">يجب اختيار الكلية أولاً</p>';
        return;
    }
    ...
    filtered.forEach(sec => {
        const div = document.createElement('div');
        div.className = "flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl hover:bg-blue-50 transition cursor-pointer border border-transparent hover:border-blue-200 group";
        const isChecked = currentSelectedScopes.has(sec.id) ? 'checked' : '';

        div.innerHTML = `
            <input type="checkbox" id="chk-${sec.id}" ${isChecked} class="w-5 h-5 accent-blue-600 rounded cursor-pointer">
            <label for="chk-${sec.id}" class="flex-grow cursor-pointer dark:text-white font-bold text-xs select-none">
                ${sec.title} 
                <span class="block text-[10px] text-gray-400 font-normal mt-0.5">${sec.departmentId}</span>
            </label>
        `;
...
const renderSelectedTags = () => {
    ...
    if (currentSelectedScopes.size === 0) {
        container.innerHTML = '<span class="text-xs text-gray-400 italic w-full text-center py-2">لم يتم تحديد مواد = مشرف عام (يملك كافة الصلاحيات على المحتوى)</span>';
        return;
    }
    ...
    currentSelectedScopes.forEach(id => {
        ...
        tag.className = "bg-blue-600 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-2 animate-fade-in shadow-sm";
...
const loadAdminsList = async () => {
    ...
    list.innerHTML = '<div class="text-center py-10"><i class="fas fa-circle-notch fa-spin text-2xl text-gray-400"></i></div>';
    ...
    snap.forEach(d => {
        ...
        if (p.users) permBadges += '<span class="text-[9px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded border border-blue-200">👥 طلاب</span> ';
        ...
        const div = document.createElement('div');
        div.className = "p-4 bg-gray-50 dark:bg-gray-700/30 rounded-2xl border dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 group transition hover:border-blue-500 hover:bg-white dark:hover:bg-gray-800 hover:shadow-md";
        div.innerHTML = `
            <div class="text-right overflow-hidden flex-1">
                ...
                <div class="flex flex-wrap gap-1 mb-2">
                    <span class="text-[9px] ${count > 0 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'} px-2 py-0.5 rounded font-bold">
                        ${count > 0 ? `محدد: ${count} مادة` : 'كل المواد'}
                    </span>
                    <span class="text-[9px] bg-blue-50 text-blue-700 px-2 py-0.5 rounded font-bold">
                        ${scopeStr}
                    </span>
                    ${data.level ? `<span class="text-[9px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded font-bold">Lv.${data.level}</span>` : ''}
                </div>
                <div class="flex flex-wrap gap-1 opacity-75">
                    ${permBadges || '<span class="text-[9px] text-gray-400">لا صلاحيات إضافية</span>'}
                </div>
            </div>
            <div class="flex gap-2">
                <button onclick="window.editAdminPermissions('${data.email}')" class="w-9 h-9 bg-blue-50 text-blue-600 rounded-xl shadow-sm hover:bg-blue-600 hover:text-white transition flex items-center justify-center" title="تعديل الصلاحيات">
...
const resetAdminForm = () => {
    ...
    document.getElementById('admin-form-title').innerHTML = '<i class="fas fa-user-plus text-blue-600"></i> إضافة مشرف جديد';
...
window.batchVerifyAll = async () => {
    ...
    progressDiv.className = 'fixed top-4 left-1/2 -translate-x-1/2 z-[9999] bg-white dark:bg-gray-800 shadow-2xl rounded-2xl p-6 w-[90%] max-w-md border-2 border-green-500';
    progressDiv.innerHTML = `
        <h3 class="font-black text-lg mb-3 dark:text-white">⏳ جاري التوثيق...</h3>
        <div class="bg-gray-200 dark:bg-gray-700 rounded-full h-4 mb-2 overflow-hidden">
            <div id="batch-progress-bar" class="bg-gradient-to-r from-green-500 to-emerald-500 h-full rounded-full transition-all duration-300" style="width: 0%"></div>
        </div>
        <p id="batch-progress-text" class="text-sm text-gray-500 font-bold">0 / ${totalCount}</p>
    `;
...
window.openAnnouncementControl = async () => {
        ...
        <div class="college-group bg-white/10 rounded-xl p-3 mb-2">
            ...
            <div class="departments-list hidden pr-6 space-y-1" id="ann-depts-${c.id}">
                <label class="flex items-center gap-2 cursor-pointer text-xs opacity-80 hover:opacity-100">
                    <input type="checkbox" name="ann-target-dept-${c.id}" value="all" class="w-3 h-3 accent-green-500" checked>
                    <span>✓ كل الأقسام</span>
                </label>
                ${c.departments.map(d => `
                    <label class="flex items-center gap-2 cursor-pointer text-xs opacity-80 hover:opacity-100">
                        <input type="checkbox" name="ann-target-dept-${c.id}" value="${d.id}" class="w-3 h-3 accent-blue-500 ann-dept-checkbox">
                        <span>${d.name}</span>
                    </label>
                `).join('')}
            </div>
    ...
    modal.innerHTML = `
        <div class="bg-white dark:bg-gray-800 w-full max-w-2xl max-h-[90vh] rounded-3xl overflow-hidden shadow-2xl animate-scale-in flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
                <!-- نوع الإعلان -->
                <div>
                    <label class="block text-sm font-bold mb-2 dark:text-gray-300">نوع الإعلان:</label>
                    <div class="grid grid-cols-4 gap-2">
                        <label class="cursor-pointer">
                            <input type="radio" name="ann-type" value="info" class="hidden peer" ${current.type === 'info' || !current.type ? 'checked' : ''}>
                            <div class="peer-checked:ring-4 peer-checked:ring-blue-500 peer-checked:scale-105 p-3 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-500 text-white text-center transition-all">
                        ...
                        <label class="cursor-pointer">
                            ...
                            <div class="peer-checked:ring-4 peer-checked:ring-green-500 peer-checked:scale-105 p-3 rounded-xl bg-gradient-to-r from-green-500 to-emerald-500 text-white text-center transition-all">
                ...
                <!-- العنوان -->
                <div>
                    <label class="block text-sm font-bold mb-2 dark:text-gray-300">عنوان الإعلان (اختياري):</label>
                    <input type="text" id="ann-title" value="${current.title || ''}" placeholder="مثال: تنبيه هام من الإدارة" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white dark:border-gray-600 outline-none focus:ring-2 focus:ring-orange-500">
                </div>

                <!-- النص -->
                <div>
                    <label class="block text-sm font-bold mb-2 dark:text-gray-300">نص الإعلان: <span class="text-red-500">*</span></label>
                    <textarea id="ann-text" rows="3" placeholder="اكتب نص الإعلان هنا..." class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white dark:border-gray-600 outline-none focus:ring-2 focus:ring-orange-500">${current.text || ''}</textarea>
                </div>

                <!-- الرابط -->
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold mb-2 dark:text-gray-300">رابط (اختياري):</label>
                        <input type="url" id="ann-link" value="${current.link || ''}" placeholder="https://..." class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white dark:border-gray-600 outline-none focus:ring-2 focus:ring-orange-500">
                    </div>
                    <div>
                        <label class="block text-sm font-bold mb-2 dark:text-gray-300">نص الرابط:</label>
                        <input type="text" id="ann-link-text" value="${current.linkText || ''}" placeholder="اعرف المزيد" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white dark:border-gray-600 outline-none focus:ring-2 focus:ring-orange-500">
                    </div>
                </div>

                <!-- الاستهداف -->
                <div class="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-2xl">
                    <label class="block text-sm font-bold mb-3 dark:text-gray-300">🎯 استهداف الإعلان:</label>
                    ...
                    <div id="ann-colleges-selection" class="${current.targetAll !== false ? 'hidden' : ''}">
                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">أو اختر كليات/أقسام محددة:</p>
                        <div class="max-h-48 overflow-y-auto custom-scrollbar bg-gray-100 dark:bg-gray-700 rounded-xl p-2">
                            ${collegesWithDepts}
                        </div>
                    </div>
                </div>
            ...
            <!-- الأزرار -->
            <div class="p-6 bg-gray-50 dark:bg-gray-700/50 flex gap-3 flex-shrink-0">
                ...
                <button id="ann-hide-btn" class="px-6 bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-white py-3 rounded-xl font-bold hover:bg-gray-300 transition">
...
window.startEditAdmin = (email, allowedSectionsStr, permissionsStr, scopeStr) => {
    try {
        ...
        editingAdminEmail = email;
        document.getElementById('admin-form-title').innerHTML = `<i class="fas fa-edit text-orange-500"></i> تعديل: <span class="text-gray-500 font-mono text-lg">${email}</span>`;
        document.getElementById('admin-email').value = email;
        document.getElementById('admin-email').disabled = true;
...
window.openBadgeManager = async (userId) => {
    ...
    listContent.innerHTML = '<div class="text-center p-4"><i class="fas fa-spinner fa-spin text-blue-600"></i></div>';
    ...
    const render = () => {
        if (badges.length === 0) listContent.innerHTML = '<p class="text-center text-sm text-gray-400 py-4">لا توجد أوسمة لهذا الطالب.</p>';
        else {
            listContent.innerHTML = badges.map((b, i) => `
                <div class="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-xl mb-2 border dark:border-gray-600 animate-fade-in">
...
window.sendUrgentAlert = async () => {
    ...
    modal.innerHTML = `
        <div class="bg-white dark:bg-gray-800 w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl animate-scale-in">
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-xs font-bold mb-2 dark:text-gray-300 uppercase tracking-wide">نوع التنبيه:</label>
                    <div class="grid grid-cols-3 gap-2">
                        <label class="cursor-pointer">
                            <input type="radio" name="urgent-type" value="info" class="hidden peer" checked>
                            <div class="peer-checked:ring-2 peer-checked:ring-blue-500 peer-checked:bg-blue-50 dark:peer-checked:bg-blue-900/30 p-3 rounded-xl text-center transition border dark:border-gray-700">
                                <i class="fas fa-info-circle text-blue-500 text-lg mb-1 block"></i>
                                <span class="text-xs font-bold dark:text-gray-300">عام</span>
                            </div>
                        </label>
                        <label class="cursor-pointer">
                            <input type="radio" name="urgent-type" value="warning" class="hidden peer">
                            <div class="peer-checked:ring-2 peer-checked:ring-amber-500 peer-checked:bg-amber-50 dark:peer-checked:bg-amber-900/30 p-3 rounded-xl text-center transition border dark:border-gray-700">
                                <i class="fas fa-exclamation-triangle text-amber-500 text-lg mb-1 block"></i>
                                <span class="text-xs font-bold dark:text-gray-300">تحذير</span>
                            </div>
                        </label>
                        <label class="cursor-pointer">
                            <input type="radio" name="urgent-type" value="danger" class="hidden peer">
                            <div class="peer-checked:ring-2 peer-checked:ring-red-500 peer-checked:bg-red-50 dark:peer-checked:bg-red-900/30 p-3 rounded-xl text-center transition border dark:border-gray-700">
                                <i class="fas fa-exclamation-circle text-red-500 text-lg mb-1 block"></i>
                                <span class="text-xs font-bold dark:text-gray-300">عاجل</span>
                            </div>
                        </label>
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-bold mb-2 dark:text-gray-300 uppercase tracking-wide">نص التنبيه: <span class="text-red-500">*</span></label>
                    <textarea id="urgent-alert-text" rows="3" placeholder="اكتب نص التنبيه العاجل هنا..." class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white dark:border-gray-600 outline-none focus:ring-2 focus:ring-red-500 text-sm"></textarea>
                </div>
...
window.loadWhitelistEmails = async () => {
    ...
    try {
        ...
        if (list.length === 0) {
            container.innerHTML = '<p class="text-center text-xs text-gray-400 py-4 opacity-50">لا توجد إيميلات مستثناة حالياً</p>';
            return;
        }

        container.innerHTML = list.map(email => `
            <div class="flex items-center justify-between bg-green-50 dark:bg-green-900/20 p-3 rounded-xl border border-green-200 dark:border-green-800">
                <span class="text-sm font-mono text-green-700 dark:text-green-300">${email}</span>
...
const renderUsersTableImpl = (users, isGlobalOwner) => {
    ...
    if (users.length === 0) {
        container.innerHTML = '<div class="p-20 text-center text-gray-400 font-bold">لا يوجد طلاب مطابقين للفلتر.</div>';
        return;
    }
            ...
            <tr class="group border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition ${isPlatformBanned ? 'bg-red-50 dark:bg-red-900/10' : ''}">
                <td class="px-6 py-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <div class="font-black dark:text-white text-sm flex items-center gap-2">
                                <a href="#profile/${userData.uid || userData.id}" class="hover:text-blue-600 transition">${userData.displayName}</a>
                                ${isVerified ? '<i class="fas fa-check-circle text-blue-500 text-xs" title="موثق"></i>' : ''}
                                ${isPlatformBanned ? '<span class="text-[9px] bg-red-600 text-white px-1.5 rounded">محظور</span>' : ''}
                                <button onclick="window.editUserName('${userData.uid || userData.id}', '${userData.displayName}')" class="text-xs text-gray-400 hover:text-blue-500 transition" title="تعديل الاسم"><i class="fas fa-pen"></i></button>
                            </div>
                            <div class="text-[10px] text-gray-500 font-mono">${isGlobalOwner ? userData.email : ''}</div>
                            ${isOnline ? '<span class="text-[10px] text-green-500 font-bold"><i class="fas fa-circle text-[6px] mr-1"></i>متصل</span>' : (userData.lastSeen ? `<span class="text-[10px] text-gray-400">${window._adminTimeAgo?.(userData.lastSeen) || ''}</span>` : '')}
                            <div class="text-[10px] text-blue-400 font-bold">${userData.collegeId || '?'} / ${userData.departmentId || '?'}</div>
                ...
                <td class="px-6 py-4">
                    <div class="flex justify-end gap-2 flex-wrap">
                        ...
                        <button onclick="window.openBonusModal('${userData.uid || userData.id}', '${userData.displayName}')" class="w-8 h-8 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition flex items-center justify-center" title="إضافة درجات"><i class="fas fa-plus"></i></button>
                        <button onclick="window.changeStudentCollege('${userData.uid || userData.id}', '${userData.displayName}')" class="w-8 h-8 bg-blue-100 text-indigo-700 rounded-lg hover:bg-blue-200 transition flex items-center justify-center" title="نقل الطالب"><i class="fas fa-exchange-alt"></i></button>
                        ...
                        <button onclick="window.toggleChatBan('${userData.uid || userData.id}', ${isChatBanned})" class="w-8 h-8 rounded-lg transition flex items-center justify-center ${isChatBanned ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/30' : 'bg-gray-100 text-gray-500 hover:bg-orange-100 hover:text-orange-600'}" title="${isChatBanned ? 'فك حظر الشات' : 'حظر الشات'}">
                            <i class="fas fa-comment-slash"></i>
                        </button>
                        ...
                        ${(isGlobalOwner || window._adminPerms?.deleteUsers) ? `
                            <button onclick="window.deleteUserPermanently('${userData.uid || userData.id}', '${userData.displayName?.replace(/'/g, "\\'") || 'هذا المستخدم'}', '${userData.email || ''}')" class="w-8 h-8 bg-gray-800 text-white rounded-lg hover:bg-black transition flex items-center justify-center shadow-lg" title="حذف نهائي ❌">
                                <i class="fas fa-times"></i>
                            </button>
                        ` : ''}
                    </div>
...
export const openGradesUploadView = async () => {
    ...
    content.innerHTML = `
        <div class="max-w-3xl mx-auto">
            <!-- تعليمات -->
            <div class="bg-gradient-to-r from-blue-500 to-indigo-600 text-white p-6 rounded-2xl mb-6 shadow-lg">
            ...
            <!-- منطقة الرفع -->
            <div class="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl border-2 border-dashed border-blue-300 dark:border-blue-700 text-center">
                <input type="file" id="grades-file-input" accept=".xlsx,.xls,.csv" class="hidden">
                <div id="grades-upload-zone" class="cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20 p-10 rounded-2xl transition" onclick="document.getElementById('grades-file-input').click()">
                    ...
                    <p class="text-gray-500 text-sm">أو اسحب الملف وأفلته هنا</p>
                </div>
                <p id="grades-file-name" class="mt-4 font-bold text-blue-600 hidden"></p>
            </div>
            ...
            <div id="grades-preview" class="hidden mt-6">
                ...
                <button onclick="window.uploadGradesToFirestore()" class="mt-6 w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white py-4 rounded-2xl font-bold text-lg shadow-lg hover:shadow-xl hover:scale-[1.02] transition">
...
window.processGradesExcel = async (file) => {
    ...
    try {
        ...
        previewBody.innerHTML = gradesData.slice(0, 20).map(r => `
            <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600/50">
                <td class="p-2 font-mono">${r.studentId}</td>
                <td class="p-2">${r.subject}</td>
                <td class="p-2 text-center font-bold text-green-600">${r.score}</td>
                <td class="p-2 text-center text-gray-500">${r.maxScore}</td>
            </tr>
        `).join('');
...
window.uploadGradesToFirestore = async () => {
    ...
    resultEl.innerHTML = '<div class="text-center p-6"><i class="fas fa-spinner fa-spin text-4xl text-blue-600"></i><p class="mt-4">جاري رفع الدرجات...</p></div>';
    resultEl.classList.remove('hidden');
    resultEl.className = 'mt-6 p-6 rounded-2xl bg-blue-50 dark:bg-blue-900/30';
    ...
    try {
        ...
        // عرض النتيجة
        resultEl.className = 'mt-6 p-6 rounded-2xl ' + (failed === 0 && notFound === 0 ? 'bg-green-100 dark:bg-green-900/30' : 'bg-yellow-100 dark:bg-yellow-900/30');
        resultEl.innerHTML = `
            <h4 class="font-bold text-lg mb-4"><i class="fas fa-check-circle text-green-600 ml-2"></i>تم الرفع!</h4>
            ...
            ${errors.length > 0 ? `<div class="bg-white dark:bg-gray-800 p-4 rounded-xl max-h-40 overflow-y-auto text-sm text-gray-600">${errors.slice(0, 20).join('<br>')}</div>` : ''}
        `;
...
window.editStudentData = async (uid) => {
    try {
        ...
        panel.innerHTML = `
            <div class="bg-white dark:bg-gray-800 rounded-3xl w-full max-w-md shadow-2xl animate-slide-up">
                <div class="p-6 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-t-3xl">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-bold mb-2 dark:text-gray-300">الاسم الرباعي</label>
                        <input type="text" id="edit-fullname" value="${u.fullName || u.displayName || ''}" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                    </div>
                    <div>
                        <label class="block text-sm font-bold mb-2 dark:text-gray-300">الرقم القومي</label>
                        <input type="text" id="edit-nationalid" value="${u.nationalId || ''}" maxlength="14" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-mono">
                    </div>
                    <div>
                        <label class="block text-sm font-bold mb-2 dark:text-gray-300">الرقم الجامعي</label>
                        <input type="text" id="edit-studentid" value="${u.studentId || ''}" maxlength="9" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-mono">
                    </div>
                    <button onclick="window.saveStudentData('${uid}')" class="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white py-4 rounded-xl font-bold hover:shadow-lg transition">
...
export const openAddExamView = async () => {
    ...
    content.innerHTML = `
        <div class="max-w-2xl mx-auto">
            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg mb-6">
                <h3 class="font-bold text-lg mb-4 dark:text-white"><i class="fas fa-plus ml-2 text-indigo-500"></i>إضافة امتحان جديد</h3>
                <div class="space-y-4">
                    <input type="text" id="exam-title" placeholder="عنوان الامتحان" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                    <input type="text" id="exam-subject" placeholder="المادة" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                    <input type="datetime-local" id="exam-date" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                    <input type="text" id="exam-location" placeholder="المكان (اختياري)" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                    <button onclick="window.addExam()" class="w-full bg-gradient-to-r from-indigo-500 to-pink-500 text-white py-4 rounded-xl font-bold hover:shadow-lg transition">
    ...
    try {
        ...
        if (examsSnap.empty) {
            list.innerHTML = '<p class="text-center text-gray-500 py-6">لا توجد امتحانات مجدولة</p>';
            return;
        }
            ...
            html += `
                <div class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow flex items-center justify-between">
                    <div>
                        ...
                        <div class="text-sm text-gray-500">${exam.subject || ''} • ${examDate.toLocaleDateString('ar-EG')}</div>
                    </div>
...
export const openQuestionBankView = async () => {
    ...
    content.innerHTML = `
        <div class="max-w-3xl mx-auto">
            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg mb-6">
                <h3 class="font-bold text-lg mb-4 dark:text-white"><i class="fas fa-plus ml-2 text-blue-500"></i>إضافة سؤال جديد</h3>
                <div class="space-y-4">
                    <input type="text" id="q-subject" placeholder="المادة" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                    <input type="text" id="q-year" placeholder="السنة (مثال: 2024)" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                    <textarea id="q-question" placeholder="نص السؤال" rows="3" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white"></textarea>
                    <textarea id="q-answer" placeholder="الإجابة النموذجية" rows="3" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white"></textarea>
                    <button onclick="window.addQuestion()" class="w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white py-4 rounded-xl font-bold hover:shadow-lg transition">
    ...
    try {
        ...
        if (questionsSnap.empty) {
            list.innerHTML = '<p class="text-center text-gray-500 py-6">لا توجد أسئلة في البنك</p>';
            return;
        }
            ...
            html += `
                <div class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow">
                    <div class="flex justify-between items-start mb-2">
                        <span class="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm font-bold">${q.subject} - ${q.year}</span>
                    ...
                    <details class="text-sm text-gray-600 dark:text-gray-400">
                        <summary class="cursor-pointer text-green-600 font-bold">عرض الإجابة</summary>
                        <p class="mt-2 p-3 bg-green-50 dark:bg-green-900/30 rounded-lg">${q.answer}</p>
                    </details>
                </div>
            `;
        });
...
export const openDeviceSessionsView = async () => {
    ...
    content.innerHTML = '<div class="text-center p-20"><i class="fas fa-spinner fa-spin text-4xl text-blue-600"></i><p class="mt-4 text-gray-500">جاري تحميل جلسات الأجهزة...</p></div>';
    ...
    try {
        ...
        content.innerHTML = `
            ...
            <div class="flex gap-3 mb-6">
                <button onclick="window.openDeviceSessionsView()" class="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold shadow-lg hover:bg-blue-700 transition flex items-center gap-2">
                    <i class="fas fa-sync-alt"></i> تحديث
                </button>
            ...
            <!-- قائمة المستخدمين -->
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-lg overflow-hidden border dark:border-gray-700">
                <table class="w-full text-sm text-right">
                    <thead class="bg-gray-50 dark:bg-gray-700/50 text-gray-500 dark:text-gray-300 font-bold uppercase text-xs">
                        <tr>
            ...
            return `
                                <tr class="group hover:bg-gray-50 dark:hover:bg-gray-700/50 transition ${isMultiple ? 'bg-orange-50 dark:bg-orange-900/10' : ''}">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <img src="${data.photo || 'https://ui-avatars.com/api/?name=' + (data.name || 'U')}" class="w-10 h-10 rounded-full border-2 border-white shadow">
                                            <div>
                                                <a href="#profile/${uid}" class="font-bold dark:text-white hover:text-blue-600">${data.name || 'مستخدم'}</a>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 text-center">
                                        <span class="px-3 py-1 rounded-full font-black text-sm ${isMultiple ? 'bg-orange-100 text-orange-600 dark:bg-orange-900/40 dark:text-orange-400' : 'bg-green-100 text-green-600'}">${sess.length}</span>
                                    </td>
                                    <td class="px-6 py-4 text-gray-500 text-xs">${lastStr}</td>
                                    <td class="px-4 py-4">
                                        <div class="flex justify-center gap-2">
                                            <button onclick="window.showUserSessions('${uid}')" class="w-8 h-8 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition flex items-center justify-center" title="عرض التفاصيل">
...
export const openBlocksDashboard = async () => {
    ...
    content.innerHTML = '<div class="text-center p-20"><i class="fas fa-spinner fa-spin text-4xl text-blue-600"></i><p class="mt-4 text-gray-500">جاري تحميل بيانات الحظر...</p></div>';
    ...
    try {
        ...
        content.innerHTML = `
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                ...
                <div class="bg-gradient-to-br from-indigo-500 to-blue-600 text-white p-5 rounded-2xl shadow-lg">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                <th class="px-6 py-4 text-right text-sm font-bold text-gray-700 dark:text-gray-300">الحاظر</th>
                                <th class="px-4 py-4 text-center text-sm font-bold text-gray-700 dark:text-gray-300">←</th>
                                <th class="px-6 py-4 text-right text-sm font-bold text-gray-700 dark:text-gray-300">المحظور</th>
                                <th class="px-4 py-4 text-center text-sm font-bold text-gray-700 dark:text-gray-300">إجراء</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y dark:divide-gray-700">
                                        ...
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                            <td class="px-6 py-4">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <a href="#profile/${blocker.blockerUid}" class="font-bold dark:text-white hover:text-blue-600">${blocker.blockerName}</a>
                                                </div>
                                            </td>
                                            <td class="px-4 py-4 text-center">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <a href="#profile/${blockedUid}" class="font-bold dark:text-white hover:text-blue-600">${blocked.name}</a>
                                                </div>
                                            </td>
                                            <td class="px-4 py-4 text-center">
                                                <button onclick="window.adminUnblock('${blocker.blockerUid}', '${blockedUid}')" class="px-3 py-1 bg-green-100 text-green-600 rounded-lg hover:bg-green-200 transition text-sm font-bold">
...
window.showUserSessions = async (userId) => {
    ...
    snap.forEach(d => {
        ...
        html += `
            <div class="bg-white dark:bg-gray-700 p-4 rounded-xl mb-3 border-r-4 border-blue-500">
                <div class="flex justify-between items-start mb-2">
                    <div class="flex items-center gap-2">
                        <i class="fas fa-${s.deviceInfo?.os?.includes('Win') ? 'laptop' : s.deviceInfo?.os?.includes('Android') ? 'mobile-alt' : 'desktop'} text-blue-500"></i>
    ...
    modal.innerHTML = `
        <div class="bg-white dark:bg-gray-800 w-full max-w-md max-h-[80vh] rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div class="p-4 overflow-y-auto max-h-[60vh]">${html || '<p class="text-center text-gray-400">لا توجد جلسات</p>'}</div>
        </div>
    `;
    document.body.appendChild(modal);
};
...
export const openLoginLogsView = async () => {
    ...
    try {
        ...
        content.innerHTML = `
            ...
            return `
                                    <tr class="group hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                        <td class="px-4 py-3">
                                            <div>
                                                <a href="#profile/${log.userId}" class="font-bold dark:text-white hover:text-blue-600">${log.userName || 'مجهول'}</a>
                                                <p class="text-xs text-gray-400">${log.userEmail || ''}</p>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 text-gray-500 text-xs">${timeStr}</td>
                                        <td class="px-4 py-3 text-gray-600 dark:text-gray-300">${log.deviceInfo?.browser || '-'}</td>
                                        <td class="px-4 py-3 text-gray-500 text-xs">${log.deviceInfo?.os || '-'}</td>
                                        <td class="px-4 py-3 text-center">
                                            <span class="px-2 py-1 rounded-full text-xs font-bold ${isNew ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'}">${isNew ? 'جديد' : 'تحديث'}</span>
                                        </td>
                                    </tr>
                                `;
        }).join('')}
        ...
        content.innerHTML = `
            ...
            <div class="grid grid-cols-3 gap-4 mb-6">
                <div class="bg-red-100 dark:bg-red-900/30 p-4 rounded-2xl text-center">
                    ...
                    <div class="text-xs text-gray-500 font-bold">بلاغ معلق</div>
                </div>
                <div class="bg-green-100 dark:bg-green-900/30 p-4 rounded-2xl text-center">
                    <div class="text-3xl font-black text-green-600">${resolved.length}</div>
                    <div class="text-xs text-gray-500 font-bold">تم حله</div>
                </div>
                <div class="bg-blue-100 dark:bg-blue-900/30 p-4 rounded-2xl text-center">
                    <div class="text-3xl font-black text-blue-600">${reportsSnap.size}</div>
                    <div class="text-xs text-gray-500 font-bold">إجمالي</div>
                </div>
            </div>
            ...
            ${pending.length === 0 ? '<p class="text-center text-gray-400 p-8 bg-green-50 dark:bg-green-900/20 rounded-2xl">🎉 لا توجد بلاغات معلقة!</p>' : `
            <div class="space-y-4">
                ${pending.map(r => `
                    <div class="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow border-r-4 border-red-500">
                        <div class="flex justify-between items-start mb-3">
                            <div>
                                <p class="font-bold dark:text-white">${r.reporterName || 'مجهول'}</p>
                                <p class="text-xs text-gray-400">${r.createdAt?.toDate?.()?.toLocaleString('ar-EG') || '-'}</p>
                            </div>
                            <span class="px-2 py-1 bg-red-100 text-red-600 rounded-full text-xs font-bold">${r.type || 'بلاغ'}</span>
                        </div>
                        <p class="text-sm text-gray-600 dark:text-gray-300 mb-3 bg-gray-50 dark:bg-gray-700 p-3 rounded-xl">${r.reason || 'لا يوجد سبب'}</p>
                        <div class="flex gap-2">
                            <button onclick="window.resolveReport('${r.id}')" class="flex-1 bg-green-600 text-white py-2 rounded-xl font-bold text-sm hover:bg-green-700 transition">
                            ...
                            <button onclick="window.deleteReport('${r.id}')" class="bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-4 py-2 rounded-xl font-bold text-sm hover:bg-gray-300 transition">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    </div>
...
window.showReportUserModal = (userId, userName) => {
    ...
    modal.innerHTML = `
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl max-w-md w-full p-6 transform scale-100">
            <div class="flex justify-between items-center mb-4">
                ...
                <button onclick="document.getElementById('report-user-modal').remove()" class="w-8 h-8 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center hover:bg-gray-200 dark:hover:bg-gray-600">
                    <i class="fas fa-times text-gray-500"></i>
                </button>
            </div>
            
            <div class="space-y-3 mb-4">
                <label class="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-xl cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600">
            ...
            <textarea id="report-other-reason" placeholder="اكتب تفاصيل إضافية (اختياري)..." class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white text-sm hidden" rows="2"></textarea>
...
export const openScheduledNotifications = async () => {
    ...
    try {
        ...
        content.innerHTML = `
            <!-- إنشاء إشعار جديد -->
            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border dark:border-gray-700 mb-6">
                <h3 class="font-black text-lg mb-4 dark:text-white">➕ إنشاء إشعار مجدول</h3>
                <div class="grid md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-xs font-bold mb-1 dark:text-gray-300">العنوان:</label>
                        <input type="text" id="sched-title" placeholder="عنوان الإشعار" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-bold">
                    </div>
                    <div>
                        <label class="block text-xs font-bold mb-1 dark:text-gray-300">الوقت المجدول:</label>
                        <input type="datetime-local" id="sched-time" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                    </div>
                </div>
                <div class="mb-4">
                    <label class="block text-xs font-bold mb-1 dark:text-gray-300">المحتوى:</label>
                    <textarea id="sched-body" placeholder="نص الإشعار..." class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white h-20"></textarea>
                </div>
                <button onclick="window.createScheduledNotification()" class="w-full bg-blue-600 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-blue-700 transition">
            ...
            ${notifsSnap.empty ? '<p class="text-center text-gray-400 p-8 bg-gray-50 dark:bg-gray-700 rounded-2xl">لا توجد إشعارات مجدولة</p>' : `
            <div class="space-y-3">
            ...
            return `
                        <div class="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow border-r-4 ${isPast ? 'border-green-500' : 'border-yellow-500'}">
                            <div class="flex justify-between items-start">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">
                                        ...
                                        ${isPast ? '<span class="text-green-500 mr-2">✓ تم الإرسال</span>' : '<span class="text-yellow-500 mr-2">⏳ قيد الانتظار</span>'}
                                    </p>
                                </div>
                                <div class="flex gap-2">
                                    ${!isPast ? `<button onclick="window.sendScheduledNow('${d.id}')" class="bg-green-600 text-white px-3 py-1 rounded-lg text-xs font-bold"><i class="fas fa-paper-plane"></i></button>` : ''}
...
export const openAdvancedExport = async () => {
    ...
    content.innerHTML = `
        <div class="grid md:grid-cols-2 gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border dark:border-gray-700">
                <h3 class="font-black text-lg mb-4 dark:text-white flex items-center gap-2">
                    <i class="fas fa-user text-blue-500"></i> تصدير بيانات مستخدم
                </h3>
                <p class="text-sm text-gray-500 mb-4">تصدير كل بيانات مستخدم معين (معلوماته، درجاته، نشاطه)</p>
                <input type="text" id="export-user-email" placeholder="إيميل المستخدم" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white mb-4 font-mono text-sm">
                <button onclick="window.exportUserData()" class="w-full bg-blue-600 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-blue-700 transition">
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border dark:border-gray-700">
                <h3 class="font-black text-lg mb-4 dark:text-white flex items-center gap-2">
                    <i class="fas fa-calendar text-green-500"></i> تصدير إحصائيات شهرية
                </h3>
                <p class="text-sm text-gray-500 mb-4">تصدير إحصائيات المنصة لشهر معين</p>
                <input type="month" id="export-month" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white mb-4">
                <button onclick="window.exportMonthlyStats()" class="w-full bg-green-600 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-green-700 transition">
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border dark:border-gray-700">
                <h3 class="font-black text-lg mb-4 dark:text-white flex items-center gap-2">
                    <i class="fas fa-database text-indigo-500"></i> تصدير كل المستخدمين
                </h3>
                <p class="text-sm text-gray-500 mb-4">تصدير قائمة بكل المستخدمين وبياناتهم الأساسية</p>
                <button onclick="window.exportAllUsers()" class="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-indigo-700 transition">
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border dark:border-gray-700">
                ...
                <p class="text-sm text-gray-500 mb-4">تصدير جميع درجات الاختبارات والواجبات</p>
...
export const openContentManagement = async () => {
    ...
    try {
        ...
        content.innerHTML = `
            ...
            <!-- شريط أدوات متقدمة -->
            <div class="mb-6 bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-lg border dark:border-gray-700">
                <div class="flex items-center justify-between mb-3">
                    <h4 class="font-black text-sm text-gray-600 dark:text-gray-300 flex items-center gap-2">
                        <i class="fas fa-tools text-blue-500"></i> أدوات متقدمة
                    </h4>
                    <span class="text-xs font-bold text-gray-400 bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full">
                        <i class="fas fa-database text-blue-500 ml-1"></i> ${snap.size} مادة
                    </span>
                </div>
                <div class="flex flex-wrap gap-2">
                    ...
                    <button onclick="window._cmOpenToolModal('move')" class="cm-tool-btn bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 hover:bg-blue-100 border-blue-200 dark:border-blue-800">
                    ...
                    <button onclick="window._cmOpenToolModal('restore')" class="cm-tool-btn bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 hover:bg-green-100 border-green-200 dark:border-green-800">
            ...
            <div id="cm-tool-modal" class="hidden fixed inset-0 bg-black/70 z-[200] flex items-center justify-center p-4 backdrop-blur-sm">
                <div class="bg-white dark:bg-gray-800 w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-scale-in">
...
window._cmBrowseContent = (collegeId = null, deptId = null) => {
    ...
    // ---- Breadcrumb ----
    html += `<div class="mb-4 flex items-center flex-wrap gap-2 bg-white dark:bg-gray-800 p-3 px-4 rounded-xl shadow-sm border dark:border-gray-700">
        <div class="flex items-center gap-2 text-sm flex-wrap">
            <button onclick="window._cmBrowseContent()" class="text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 px-2 py-1 rounded-lg font-bold flex items-center gap-1 transition">
                <i class="fas fa-home"></i> الرئيسية
            </button>`;

    if (collegeId) {
        const college = UNIVERSITY_STRUCTURE.find(c => c.id === collegeId);
        html += `<i class="fas fa-chevron-left text-gray-300 text-xs"></i>
            <button onclick="window._cmBrowseContent('${collegeId}')" class="${deptId ? 'text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30' : 'text-gray-600 dark:text-gray-300'} px-2 py-1 rounded-lg font-bold transition">
    ...
    if (deptId && collegeId) {
        ...
        html += `<i class="fas fa-chevron-left text-gray-300 text-xs"></i>
            <span class="text-gray-600 dark:text-gray-300 font-bold px-2 py-1">📁 ${dept?.name || deptId}</span>`;
    }
    html += `</div></div>`;
    ...
    if (!collegeId) {
        ...
        if (window._cmGeneralSections.length > 0) {
            html += `<div onclick="window._cmShowGeneralSections()" class="cm-folder bg-gradient-to-br from-emerald-600 to-blue-700 group">
        ...
        UNIVERSITY_STRUCTURE.forEach(college => {
            ...
            html += `<div onclick="window._cmBrowseContent('${college.id}')" class="cm-folder ${count > 0 ? 'bg-gradient-to-br from-blue-600 to-indigo-700' : 'bg-gradient-to-br from-gray-500 to-gray-600'} group">
                <div class="w-14 h-14 bg-white/15 rounded-2xl flex items-center justify-center mb-3 group-hover:scale-110 transition mx-auto">
        ...
        html += `<div class="mb-3 text-sm font-bold text-gray-400"><i class="fas fa-university text-blue-500 ml-1"></i> ${totalCollegeSections} مادة في هذه الكلية</div>`;
        ...
        college.departments.forEach(dept => {
            ...
            html += `<div onclick="window._cmBrowseContent('${collegeId}', '${dept.id}')" class="cm-folder ${sections.length > 0 ? 'bg-gradient-to-br from-indigo-600 to-blue-700' : 'bg-gradient-to-br from-gray-500 to-gray-600'} group">
                <div class="w-14 h-14 bg-white/15 rounded-2xl flex items-center justify-center mb-3 group-hover:scale-110 transition mx-auto">
        ...
        html += `<div class="mb-3 flex items-center gap-3">
            <div class="flex-1 relative">
                <i class="fas fa-search absolute top-1/2 -translate-y-1/2 right-3 text-gray-400 text-sm"></i>
                <input type="text" id="cm-search-input" placeholder="بحث في المواد..." class="w-full p-2.5 pr-9 border rounded-xl dark:bg-gray-700 dark:text-white dark:border-gray-600 text-sm outline-none focus:ring-2 focus:ring-blue-500" oninput="window._cmFilterSearch()">
            </div>
            <span class="text-sm font-bold text-gray-400 whitespace-nowrap">${sections.length} مادة</span>
        </div>`;

        if (sections.length === 0) {
            html += `<div class="text-center py-16 bg-gray-50 dark:bg-gray-800 rounded-2xl border-2 border-dashed dark:border-gray-700">
                <i class="fas fa-folder-open text-5xl text-gray-300 dark:text-gray-600 mb-4"></i>
                <p class="text-gray-400 font-bold">لا توجد مواد في هذا القسم</p>
            </div>`;
        } else {
            ...
            sections.forEach(s => {
                ...
                html += `<div class="cm-section-card flex justify-between items-center p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md hover:border-blue-400 transition group" data-title="${s.title}">
                    <div class="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="flex items-center gap-2 mt-0.5">
                                ${createdAt ? `<span class="text-[10px] text-gray-400"><i class="far fa-calendar-alt ml-1"></i>${createdAt}</span>` : ''}
                                <span class="text-[10px] text-blue-500 bg-blue-50 dark:bg-blue-900/20 px-1.5 py-0.5 rounded">${targetInfo}</span>
                            </div>
                        </div>
                    </div>
                    <div class="flex gap-1.5 mr-2">
                        <button onclick="window.location.hash='admin/cms/${s.id}'" class="w-9 h-9 rounded-lg bg-blue-50 dark:bg-blue-900/30 text-blue-600 hover:bg-blue-600 hover:text-white transition flex items-center justify-center text-sm" title="فتح الدروس"><i class="fas fa-folder-open"></i></button>
...
window._cmShowGeneralSections = () => {
    ...
    let html = `<div class="mb-4 flex items-center flex-wrap gap-2 bg-white dark:bg-gray-800 p-3 px-4 rounded-xl shadow-sm border dark:border-gray-700">
        <div class="flex items-center gap-2 text-sm">
            <button onclick="window._cmBrowseContent()" class="text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 px-2 py-1 rounded-lg font-bold flex items-center gap-1 transition">
                <i class="fas fa-home"></i> الرئيسية
            </button>
            <i class="fas fa-chevron-left text-gray-300 text-xs"></i>
            <span class="text-gray-600 dark:text-gray-300 font-bold px-2 py-1">📚 المواد العامة</span>
        </div>
        <span class="mr-auto text-sm font-bold text-gray-400">${window._cmGeneralSections.length} مادة</span>
    </div>`;
    ...
    html += `<div class="mb-3">
        <div class="relative">
            <i class="fas fa-search absolute top-1/2 -translate-y-1/2 right-3 text-gray-400 text-sm"></i>
            <input type="text" id="cm-search-input" placeholder="بحث في المواد العامة..." class="w-full p-2.5 pr-9 border rounded-xl dark:bg-gray-700 dark:text-white dark:border-gray-600 text-sm outline-none focus:ring-2 focus:ring-blue-500" oninput="window._cmFilterSearch()">
        </div>
    </div>`;
    ...
    window._cmGeneralSections.forEach(s => {
        const createdAt = s.createdAt?.toDate ? s.createdAt.toDate().toLocaleDateString('ar-EG', { year: 'numeric', month: 'short', day: 'numeric' }) : '';
        html += `<div class="cm-section-card flex justify-between items-center p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md hover:border-emerald-400 transition group" data-title="${s.title}">
            <div class="flex items-center gap-3 flex-1 min-w-0">
                <div class="w-11 h-11 bg-emerald-50 dark:bg-emerald-900/30 rounded-xl flex items-center justify-center flex-shrink-0">
                ...
                <div class="min-w-0">
                    ...
                    <div class="flex items-center gap-2 mt-0.5">
                        ${createdAt ? `<span class="text-[10px] text-gray-400"><i class="far fa-calendar-alt ml-1"></i>${createdAt}</span>` : ''}
                        <span class="text-[10px] text-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 px-1.5 py-0.5 rounded">كل الكليات</span>
                    </div>
                </div>
            </div>
            <div class="flex gap-1.5 mr-2">
...
window._cmOpenToolModal = async (tool) => {
    ...
    if (tool === 'archive') {
        ...
        body.innerHTML = `
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">سيتم نقل المادة للأرشيف وإخفائها عن الطلاب. يمكنك استعادتها لاحقاً.</p>
            <select id="cm-archive-select" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white mb-4 font-bold">
        ...
        header.className = 'p-5 text-white flex items-center justify-between bg-gradient-to-r from-blue-600 to-indigo-600';
        title.innerHTML = '<i class="fas fa-exchange-alt ml-2"></i> نقل موضوعات';
        body.innerHTML = `
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">نقل كل الدروس من مادة إلى مادة أخرى.</p>
            <label class="block text-xs font-bold text-gray-500 mb-1">من مادة:</label>
            <select id="cm-move-from" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white mb-3 font-bold">
                <option value="">اختر المصدر</option>${sectionOptions}
            </select>
            <label class="block text-xs font-bold text-gray-500 mb-1">إلى مادة:</label>
            <select id="cm-move-to" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white mb-4 font-bold">
                <option value="">اختر الوجهة</option>${sectionOptions}
            </select>
            <button onclick="window._cmDoMove()" class="w-full bg-blue-600 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-blue-700 transition flex items-center justify-center gap-2">
        ...
        body.innerHTML = `
            ...
            <select id="cm-delete-select" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white mb-4 font-bold">
                <option value="">اختر المادة</option>${sectionOptions}
            </select>
        ...
        header.className = 'p-5 text-white flex items-center justify-between bg-gradient-to-r from-green-600 to-emerald-600';
        title.innerHTML = '<i class="fas fa-undo-alt ml-2"></i> استعادة من الأرشيف';
        body.innerHTML = '<div class="text-center p-6"><i class="fas fa-spinner fa-spin text-2xl text-green-600"></i></div>';
        ...
        try {
            ...
            if (archivedSnap.empty) {
                body.innerHTML = `<div class="text-center py-8">
                    ...
                    <p class="text-gray-400 font-bold">لا توجد مواد مؤرشفة</p>
                </div>`;
            } else {
                ...
                archivedSnap.forEach(d => {
                    ...
                    archiveHtml += `<div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-xl">
                        <div>
                            <p class="font-bold text-sm dark:text-white">${data.title || d.id}</p>
                            ${archivedDate ? `<p class="text-[10px] text-gray-400">أُرشفت: ${archivedDate}</p>` : ''}
                        </div>
                        <button onclick="window._cmRestoreArchived('${d.id}')" class="bg-green-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-green-700 transition">
    ...
    try {
        ...
        content.innerHTML = `
            <div class="grid md:grid-cols-2 gap-6 mb-8">
                ...
                <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border dark:border-gray-700">
                    <h3 class="font-black text-lg mb-4 dark:text-white flex items-center gap-2">
                        <i class="fas fa-user-plus text-green-500"></i> إضافة محاولات لطالب
                    </h3>
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-bold mb-1 dark:text-gray-300">الكويز:</label>
                            <select id="attempt-quiz" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                                <option value="">اختر الكويز</option>
                                ${quizOptions}
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-bold mb-1 dark:text-gray-300">إيميل الطالب:</label>
                            <input type="email" id="attempt-email" placeholder="student@email.com" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-mono text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold mb-1 dark:text-gray-300">عدد المحاولات الإضافية:</label>
                            <input type="number" id="attempt-count" value="1" min="1" max="10" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                        </div>
                        <button onclick="window.grantExtraAttempts()" class="w-full bg-green-600 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-green-700 transition">
                ...
                <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border dark:border-gray-700">
                    <h3 class="font-black text-lg mb-4 dark:text-white flex items-center gap-2">
                        <i class="fas fa-undo text-blue-500"></i> إلغاء نتيجة طالب
                    </h3>
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-bold mb-1 dark:text-gray-300">الكويز:</label>
                            <select id="reset-quiz" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                                <option value="">اختر الكويز</option>
                                ${quizOptions}
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-bold mb-1 dark:text-gray-300">إيميل الطالب:</label>
                            <input type="email" id="reset-email" placeholder="student@email.com" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-mono text-sm">
                        </div>
                        <button onclick="window.resetStudentQuiz()" class="w-full bg-blue-600 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-blue-700 transition">
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border dark:border-gray-700">
                ...
                <div class="grid md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-bold mb-1 dark:text-gray-300">الكويز:</label>
                        <select id="college-reset-quiz" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                            <option value="">اختر الكويز</option>
                            ${quizOptions}
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-bold mb-1 dark:text-gray-300">الكلية:</label>
                        <select id="college-reset-id" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                            <option value="">اختر الكلية</option>
                            ${UNIVERSITY_STRUCTURE.map(c => `<option value="${c.id}">${c.name}</option>`).join('')}
                        </select>
                    </div>
                ...
                <p class="text-xs text-gray-400 mt-3">⚠️ سيتم حذف جميع نتائج طلاب الكلية المختارة في هذا الكويز</p>
            </div>
        `;
    } catch (e) {
...
export const openScheduledAnnouncementsView = async () => {
    ...
    content.innerHTML = `
        <div class="max-w-2xl mx-auto">
            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg mb-6">
                <h3 class="font-bold text-lg mb-4 dark:text-white"><i class="fas fa-plus ml-2 text-blue-500"></i>جدولة إعلان جديد</h3>
                <div class="space-y-4">
                    <input type="text" id="sched-title" placeholder="عنوان الإعلان" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                    <textarea id="sched-content" placeholder="محتوى الإعلان" rows="3" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white"></textarea>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-bold mb-2 dark:text-gray-300">تاريخ النشر</label>
                            <input type="datetime-local" id="sched-date" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                        </div>
                        <div>
                            <label class="block text-sm font-bold mb-2 dark:text-gray-300">النوع</label>
                            <select id="sched-type" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                    ...
                    <button onclick="window.scheduleAnnouncement()" class="w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white py-4 rounded-xl font-bold hover:shadow-lg transition">
    ...
    try {
        ...
        if (scheduledSnap.empty) {
            list.innerHTML = '<p class="text-center text-gray-500 py-6">لا توجد إعلانات مجدولة</p>';
            return;
        }
        ...
        scheduledSnap.forEach(d => {
            ...
            html += `
                <div class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow border-r-4 border-${color}-500">
                    <div class="flex justify-between items-start mb-2">
                        <div>
                            <h4 class="font-bold dark:text-white">${ann.title}</h4>
                            <p class="text-sm text-gray-500">${pubDate.toLocaleString('ar-EG')}</p>
                        </div>
                        <div class="flex gap-2 items-center">
                            <span class="px-2 py-1 ${isPending ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'} rounded-full text-xs font-bold">
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400">${ann.content}</p>
                </div>
            `;
        });
        html += '</div>';
...
export const openBannedUsersDashboard = async () => {
    ...
    content.innerHTML = '<div class="text-center p-20"><i class="fas fa-spinner fa-spin text-4xl text-red-600"></i><p class="mt-4 text-gray-500">جاري تحميل الأعضاء المحظورين...</p></div>';
    ...
    try {
        ...
        if (usersSnap.empty) {
            content.innerHTML = `
                <div class="text-center py-20 px-6">
                    ...
                    <h3 class="text-2xl font-black text-gray-700 dark:text-white mb-2">لا يوجد أعضاء محظورين</h3>
                    <p class="text-gray-500">كل الأعضاء في حالة جيدة 🎉</p>
                </div>
            `;
            return;
        }
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden">
                <div class="p-4 bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600">
                    <input type="text" id="search-banned" placeholder="بحث في المحظورين..." 
                           class="w-full p-3 border rounded-xl dark:bg-gray-600 dark:text-white"
                           oninput="window.filterBannedUsers(this.value)">
                </div>
                
                <div id="banned-users-list" class="divide-y dark:divide-gray-700">
        `;
        ...
        usersSnap.forEach(d => {
            ...
            html += `
                <div class="banned-user-item p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition flex items-center justify-between gap-4"
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 font-mono">${isOwner ? (u.email || '-') : 'مخفي'}</p>
                    ...
                    <div class="flex items-center gap-2">
                        <button onclick="window.viewBannedUserProfile('${d.id}')" 
                                class="bg-gray-100 dark:bg-gray-600 text-gray-600 dark:text-gray-300 px-3 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-500 transition text-sm font-bold">
                            <i class="fas fa-eye ml-1"></i> الملف
                        </button>
                        <button onclick="window.unbanUser('${d.id}', '${u.displayName?.replace(/'/g, "\\'")}' || 'هذا العضو')" 
                                class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition text-sm font-bold shadow-lg">
...
export const openBroadcastNotificationPanel = async () => {
        ...
        <div class="max-w-2xl mx-auto">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 space-y-6">
                ...
                <div>
                    ...
                    <select id="notif-type" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                        <option value="info">💡 معلومة</option>
                ...
                <div>
                    ...
                    <textarea id="notif-message" rows="3" placeholder="اكتب محتوى الإشعار..."
                              class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white resize-none"></textarea>
                </div>
                
                <!-- المستهدفين -->
                <div class="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl">
                    <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-3">
                        <i class="fas fa-users text-orange-500 mr-1"></i> إرسال إلى:
                    </label>
                    
                    <div class="grid grid-cols-2 gap-3 mb-4">
                        <label class="flex items-center gap-2 p-3 bg-white dark:bg-gray-800 rounded-xl cursor-pointer border-2 border-transparent has-[:checked]:border-orange-500 transition">
                    ...
                    <div id="college-select-wrap" class="hidden mb-3">
                        <select id="notif-college" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                            ${collegeOptions}
                        </select>
                    </div>
                    ...
                    <div id="dept-select-wrap" class="hidden mb-3">
                        <select id="notif-dept" class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                    ...
                    <div id="student-input-wrap" class="hidden">
                        <input type="text" id="notif-student-id" placeholder="أدخل الرقم الجامعي للطالب..."
                               class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-mono">
                    </div>
                </div>
                
                <!-- معاينة العدد -->
                <div id="target-count" class="bg-blue-50 dark:bg-blue-900/30 p-4 rounded-xl text-center">
                    <span class="text-blue-600 dark:text-blue-400 font-bold">سيصل الإشعار لجميع الطلاب في المنصة</span>
                </div>
    ...
    document.querySelectorAll('input[name="target"]').forEach(radio => {
        radio.onchange = () => {
            ...
            if (val === 'all') {
                document.getElementById('target-count').innerHTML = '<span class="text-blue-600 dark:text-blue-400 font-bold">سيصل الإشعار لجميع الطلاب في المنصة</span>';
            } else if (val === 'student') {
                document.getElementById('target-count').innerHTML = '<span class="text-indigo-600 dark:text-indigo-400 font-bold">سيصل الإشعار للطالب المحدد</span>';
            }
        };
    });
    ...
    document.getElementById('notif-college').onchange = (e) => {
        ...
        document.getElementById('target-count').innerHTML = `<span class="text-green-600 dark:text-green-400 font-bold">سيصل الإشعار لطلاب الكلية المحددة</span>`;
    };

    document.getElementById('notif-dept').onchange = () => {
        document.getElementById('target-count').innerHTML = `<span class="text-blue-600 dark:text-blue-400 font-bold">سيصل الإشعار لطلاب القسم المحدد</span>`;
    };
};
...
window.editMyProfileData = async () => {
    ...
    modal.innerHTML = `
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl max-w-md w-full overflow-hidden">
            <div class="bg-gradient-to-r from-blue-600 to-blue-600 p-6 text-white text-center">
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-1">الاسم الرباعي</label>
                    <input type="text" id="edit-fullName" value="${data.fullName || data.displayName || ''}" 
                           class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-1">الرقم القومي</label>
                    <input type="text" id="edit-nationalId" value="${data.nationalId || ''}" 
                           class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-mono text-left" maxlength="14">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-1">الرقم الجامعي (Student ID)</label>
                    <input type="text" id="edit-studentId" value="${data.studentId || ''}" 
                           class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white font-mono text-left">
                </div>
                
                <div class="flex gap-3 pt-4">
                    <button id="save-profile-btn" class="flex-1 bg-gradient-to-r from-blue-600 to-blue-600 text-white py-3 rounded-xl font-bold hover:shadow-lg transition">
                        <i class="fas fa-save ml-1"></i> حفظ
                    </button>
                    <button onclick="this.closest('.fixed').remove()" class="px-6 bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 py-3 rounded-xl font-bold hover:bg-gray-300 transition">
                        إلغاء
                    </button>
                </div>
            </div>
    ...
    modal.innerHTML = `
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl max-w-md w-full overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-1">الرقم القومي (14 رقم)</label>
                ...
                <div class="flex items-center gap-3 p-3 bg-blue-50 dark:bg-blue-900/30 rounded-xl">
                    <input type="checkbox" id="admin-edit-isVerified" ${data.isVerified ? 'checked' : ''} 
                           class="w-5 h-5 accent-blue-600 rounded">
                    <label for="admin-edit-isVerified" class="text-sm font-bold text-gray-600 dark:text-gray-300 cursor-pointer">
                        <i class="fas fa-check-circle text-blue-500 mr-1"></i> حساب موثق
                    </label>
                </div>
...
window.openAdminActivityLog = async () => {
    ...
    content.innerHTML = '<div class="text-center py-12"><i class="fas fa-spinner fa-spin text-4xl text-blue-500"></i></div>';
        ...
        logs.forEach(log => {
            ...
            rows += `
                <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                    ...
                    <td class="px-4 py-3 text-sm font-mono text-gray-600 dark:text-gray-400">${log.adminEmail}</td>
                    <td class="px-4 py-3 text-sm dark:text-gray-300">${log.details || '-'}</td>
                    <td class="px-4 py-3 text-xs text-gray-400">${timeAgo(log.timestamp)}</td>
                </tr>`;
        });

        content.innerHTML = `
            <div class="mb-4 flex items-center justify-between">
                <p class="text-sm text-gray-500">آخر ${logs.length} عملية</p>
                <button onclick="window.openUsersLogView()" class="text-sm text-blue-600 hover:underline"><i class="fas fa-arrow-right"></i> رجوع</button>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow border dark:border-gray-700 overflow-x-auto">
                <table class="w-full text-right" dir="rtl">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th class="px-4 py-3 text-xs font-bold text-gray-500">العملية</th>
                            <th class="px-4 py-3 text-xs font-bold text-gray-500">المشرف</th>
                            <th class="px-4 py-3 text-xs font-bold text-gray-500">التفاصيل</th>
                            <th class="px-4 py-3 text-xs font-bold text-gray-500">الوقت</th>
                        </tr>
                    </thead>
                    <tbody>${rows || '<tr><td colspan="4" class="text-center py-8 text-gray-400">لا يوجد سجلات بعد</td></tr>'}</tbody>
                </table>
            </div>
        `;
    } catch (e) {
...
window.openWatermarkExemptions = async () => {
    ...
    area.innerHTML = `
        <div class="max-w-2xl mx-auto">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-2xl font-black dark:text-white flex items-center gap-3">
                    <div class="w-12 h-12 rounded-2xl bg-indigo-500/20 flex items-center justify-center">
                        <i class="fas fa-eye-slash text-indigo-500 text-xl"></i>
                    </div>
                    إعفاء من العلامة المائية
                </h2>
                <button onclick="window.openAdminPanel()" class="bg-gray-200 dark:bg-gray-700 px-4 py-2 rounded-xl font-bold hover:bg-gray-300 dark:hover:bg-gray-600 transition text-sm">
            ...
            <!-- إضافة إيميل جديد -->
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-5 mb-6 border dark:border-gray-700">
                <h3 class="font-bold text-lg dark:text-white mb-4"><i class="fas fa-plus-circle text-indigo-500 ml-2"></i>إضافة إيميل معفى</h3>
                <div class="flex gap-3">
                    <input type="email" id="wm-exempt-email" placeholder="example@gmail.com" 
                        class="flex-1 p-3 border dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 font-mono text-sm" dir="ltr">
                    <button onclick="window.addWatermarkExemption()" id="wm-add-btn"
                        class="bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-indigo-700 transition whitespace-nowrap">
            ...
            <!-- القائمة -->
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-5 border dark:border-gray-700">
                <h3 class="font-bold text-lg dark:text-white mb-4"><i class="fas fa-list text-indigo-500 ml-2"></i>الإيميلات المعفاة</h3>
                <div id="wm-exempt-list" class="space-y-2">
                    <p class="text-center text-gray-400 py-4"><i class="fas fa-spinner fa-spin"></i> جاري التحميل...</p>
                </div>
            </div>
        </div>
    `;
...
async function loadWatermarkExemptions() {
    ...
    try {
        ...
        if (emails.length === 0) {
            listEl.innerHTML = '<p class="text-center text-gray-400 py-6"><i class="fas fa-check-circle text-green-400 ml-1"></i> لا يوجد إيميلات معفاة حالياً</p>';
            return;
        }

        listEl.innerHTML = emails.map((email, idx) => `
            <div class="flex items-center justify-between p-3 rounded-xl bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700 transition group">
                <div class="flex items-center gap-3">
